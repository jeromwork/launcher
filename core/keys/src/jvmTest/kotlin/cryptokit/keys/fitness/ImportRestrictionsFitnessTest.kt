...
class ImportRestrictionsFitnessTest {
    ...
    fun appCodeOutsideAdaptersMustNotImportKeysApiInternal() {
        // cryptokit.keys.api.internal.* — это backend-adapter SPI: ConfigCipher2,
        // RecipientResolver, EnvelopeStorage, PublicKeyDirectory, DeviceIdentity.
        // Caller code должен использовать только cryptokit.keys.api.RemoteStorage и
        ...
        assertNoViolations(
            violations,
            ruleName = "Encapsulation — no cryptokit.keys.api.internal in app/src/main",
            hint = "Use cryptokit.keys.api.RemoteStorage (public facade). " +
                "If you need a port from cryptokit.keys.api.internal, the adapter must live in app/src/realBackend (or mockBackend)."
        )
    }
    ...
    companion object {
         ...
         * cryptokit.keys.api.RemoteStorage facade, not via these internal contracts.
         */
