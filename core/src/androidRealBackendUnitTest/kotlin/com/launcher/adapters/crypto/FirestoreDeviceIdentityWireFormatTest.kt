...
class FirestoreDeviceIdentityWireFormatTest {
    ...
    @Test
    fun toMap_stamps_current_wire_version() {
        assertEquals(
            FirestoreDeviceIdentityRepository.WIRE_SCHEMA_VERSION,
            FirestoreDeviceIdentityRepository.toMap(sample())["schemaVersion"],
        )
    }

    @Test
    fun fromMap_rejects_unknown_future_version() {
        val map = FirestoreDeviceIdentityRepository.toMap(sample()).toMutableMap()
        map["createdAt"] = 1_700_000_000_000L
        map["schemaVersion"] = FirestoreDeviceIdentityRepository.WIRE_SCHEMA_VERSION + 998
        assertNull(FirestoreDeviceIdentityRepository.fromMap(map))
    }

    @Test
    fun fromMap_reads_a_hand_built_v1_document() {
        val id = sample()
