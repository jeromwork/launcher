...
description: ОБЯЗАТЕЛЬНО вызывать перед `gh pr create` / любым другим открытием PR на feature-ветке, которая ссылается на spec (`specs/NNN-slug/`) или на backlog-task (`task-N - ...md`). Синхронизирует Acceptance Criteria в `backlog/tasks/` по hybrid-модели: `[hand]` AC (author-written, project-specific behaviour) проставляются по grep/owner-confirm; `[auto:deferred-*]` AC регенерируются из `[deferred-*]` маркеров в `tasks.md`. Решает статус: все [x] → Done; есть [ ] среди deferred → Verification (PR merged, ждём железа); есть [ ] среди hand → In Progress (фича не закончена, PR open нельзя). Также refuse'ит если в рабочем дереве присутствуют файлы `specs/<NNN>/checklists/*.md` (правило 2026-07-16: чек-листы — chat-only, файлы не commit'аем). Legacy `[auto:checklist]` строки на уже committed tasks не трогаем. Без вызова этого skill'а — backlog рассинхронизирован с реальностью (incident PR #21/#22 → TASK-3/TASK-4 ушли в Done с 0/N AC; incident 2026-06-24 TASK-49 → 8/8 проставлено на слово при 12/45 deferred tasks).
---
...
## Hybrid AC модель (revised 2026-07-16)
...
- [ ] #4 [auto:deferred-physical-device] Physical device Xiaomi 11T (T041)
<!-- AC:END -->
```
...
AC **без маркера** — legacy формат; при первом sync классифицируется как `[hand]` (если не очевидно из текста что это deferred).

**Legacy `[auto:checklist]` AC** (existed in tasks committed before 2026-07-16) — retired concept. Do NOT regenerate; do NOT re-count. If encountered on an already-committed task: leave as-is (historical). If encountered on a new task being sync'd for the first time — REFUSE and ask owner to convert to `[hand]` or delete (checklists no longer produce persisted file counts per ADR-011 §5 revised).

## Status decision matrix

| Состояние AC | Статус |
|---|---|
| Все `[hand]` + `[auto:deferred-*]` зелёные (`[x]` или `[N/A]`) | **Done** |
| Все `[hand]` зелёные; есть `[ ]` среди `[auto:deferred-*]` | **Verification** (PR merged) |
| Есть `[ ]` среди `[hand]` | **In Progress** (PR open нельзя — фича не закончена) |
...
**4a. Refuse gate — no checklist files.** Проверить рабочее дерево: `git ls-files -o --exclude-standard specs/<NNN>/checklists/ 2>/dev/null` + `ls specs/<NNN>/checklists/ 2>/dev/null`. Если найдены файлы — **REFUSE** и попросить владельца удалить их (per ADR-011 §5 revised 2026-07-16: чек-листы — chat-only, файлы не commit'аем; `.gitignore` уже покрывает `specs/**/checklists/`, но untracked файлы могли остаться от run buffer'а). После удаления — retry.

Legacy `[auto:checklist]` строки на уже committed tasks — не трогаем, не регенерируем, не удаляем.
