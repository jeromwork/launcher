...
- Если уже идёт mentor mode и пользователь ответил на все вопросы текущего шага — переходи к шагам 6-9 без повторного «приветствия».
...
5. **Уточняющие вопросы — столько, сколько нужно теме (organic count, не фиксированное число).** Per constitution Article XIX (Organic Question Budgets): не подбивай к 5, не урезай ниже того, что тема реально требует.
   - Типичный диапазон: **3–7** вопросов.
   - < 3 — ОК если тема узкая; не выдумывай grey zones чтобы дотянуть.
   - > 8 — предложи split на два прохода: «У меня N серьёзных вопросов. Все сразу или разбить?» Пусть пользователь решит.
   - Каждый вопрос — точечный (уровень опыта, целевой артефакт, ограничения, критерии успеха), не общий («что хочешь?»). С коротким объяснением, **зачем спрашиваешь**.
   - **Padding to a target = bug.** Trim ниже organic count = leak архитектурного риска. Оба — failure mode.
...
- **Пользователь ответил неполно** на шаге 5 (например, ответил только на часть заданных вопросов). Не дави — переходи к шагу 6 с пометкой «отвечаю на основе известного; недостающие вопросы оставлю открытыми».
