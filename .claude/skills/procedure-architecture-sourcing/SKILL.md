...
1. **Prior-art research FIRST — industry standards + use cases, BEFORE our tasks (the order is load-bearing).** Research two things up front: (a) **industry standards** — how complete/working projects solved it (published specs / RFCs / whitepapers / threat models / audits / the real libs), and (b) **the use cases** — what the feature must actually do. **Only AFTER that** do you walk our own tasks/decisions, as *secondary validation* — the research may confirm, refine, or **override** them. An "open question" (a task with no decision) is NOT "hold a mentor session to invent an architecture" — the architecture is answered by researching the standard + use cases; that resolution comes first. **Never read copyleft (AGPL/GPL) source to reimplement** — clean-room from published design; copyright protects code, not architecture. Timebox: research terminates in an architecture + a build-list, not perpetual comparison. Residual choices the standard can't answer (segment defaults, product-scope tradeoffs) are the ONLY thing that goes to `mentor` — not the architecture.
...
- **Ground in researched prior art, NOT in our own past decisions — THE second load-bearing rule.** The architecture (the HOW) comes from **researched ready solutions of whole/complete projects**, in priority order: (1) ready code/lib to import, (2) ready architecture to copy clean-room from a real project (Phoenix/Wire/Signal/openmls/RFC/MIMI…), (3) our own design only if neither exists. **Our prior internal Decision blocks are NOT the source of truth** — they are secondary and MUST be **re-validated against the research**: where a past decision IS the researched best-practice, keep it (framed as "this is what real projects do", not "we decided it"); where it was AI-defaulted or diverges from best prior art, **the research wins**. Distinguish **ARCHITECTURE (the HOW — from prior art)** from **PRODUCT POLICY (the WHAT/constraints — ours: zero-knowledge, presets, MVP scope)** — the latter stays, but is marked explicitly as our constraint, laid *on top of* the researched architecture, never passed off as universal.
