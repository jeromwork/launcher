...
## 11. Cross-cutting architecture SoT + backlog alignment

Крупные архитектурные области с высокой связностью между задачами (крипто-стек, backend-топология, UX-конвенции, i18n) ведутся как **living Source of Truth документ** в `docs/dev/*-mentor-overview.md` (или аналог). Backlog task'и **не дублируют** содержимое SoT — ссылаются через frontmatter поля.

**Правило (HARD RULE для AI, введено 2026-07-02)**: после **каждой mentor-сессии**, добавившей решение в SoT (например новую секцию `Δ.N` / `Π.N` / `Ρ.N` в `docs/dev/crypto-mentor-overview.md`), AI обязан выполнить **три action'а в том же коммите** (или в следующем сразу):

1. **Update SoT документ** — добавить новую секцию с якорным ID (`### Δ.12: <название>`).
2. **Update open-questions register** (`docs/dev/crypto-open-questions.md` для крипто, аналогично для других доменов): пометить закрытые вопросы 🟢 с link'ом на секцию SoT; добавить новые всплывшие вопросы с Q-NN идентификаторами и session-tag'ами.
3. **Опционально запустить alignment sweep** — `procedure-crypto-alignment-sweep` (или аналог для другого домена), batch 10-15 задач, обновить `crypto-alignment` frontmatter marker.

**Скилы для крипто-домена** (первый реализованный кейс паттерна):
- `docs/dev/crypto-mentor-overview.md` — SoT крипто-архитектуры.
- `docs/dev/crypto-open-questions.md` — реестр вопросов Q-01..Q-NN.
- `.claude/skills/procedure-crypto-alignment-sweep/SKILL.md` — inкрементальный sweep backlog.

**Backlog task frontmatter** (расширение к CLAUDE.md §Portfolio tracker):

```yaml
crypto-alignment: aligned | needs-review | scope-reset | needs-desc-update | new-from-mentor | parked | blocked-on-question
crypto-source: [Ξ.5, Π.7, Ρ.4]           # секции crypto-mentor-overview.md что информируют задачу
blocks-on: [Q-09, Q-12]                   # open-questions что блокируют (если blocked-on-question)
crypto-sweep-date: 2026-07-02             # последний sweep timestamp
```

Значения:
- `aligned` — description соответствует текущему SoT, ничего не делать.
- `needs-review` — legacy default для не-sweep'нутых.
- `scope-reset` — feature approach устарел, major rewrite нужен.
- `needs-desc-update` — feature scope верный, детали нужно освежить.
- `new-from-mentor` — задача должна появиться из mentor-сессии (создать через `backlog task create`).
- `parked` — дубликат SoT или другой задачи, кандидат на close/merge.
- `blocked-on-question` — ждёт разрешения Q-NN из open-questions.

**Refuse pattern**: если AI пишет секцию в SoT (mentor-overview) **но не обновляет** open-questions register в тот же коммит — это regression pattern (SoT растёт, backlog отстаёт). Alternative: перед commit'ом mentor-overview изменения — обязательно update open-questions + опционально sweep.

**Универсальность**: паттерн `SoT + open-questions + alignment sweep` применим к другим cross-cutting доменам когда они начнут расти:
- Backend architecture (`docs/dev/server-roadmap.md` уже частично играет роль SoT — расширить когда backend станет активной темой).
- UX conventions (пока нет consolidated файла — создать когда наберётся ≥ 5 UX one-way-door решений).
- Localization strategy (когда пойдут ≥ 3 языка).

Для нового домена — копируется skill `procedure-crypto-alignment-sweep`, переименовывается, adjustment на domain-specific SoT.
...
17. Секция добавлена в `docs/dev/crypto-mentor-overview.md` (или другой SoT) **без** соответствующего update `docs/dev/crypto-open-questions.md` (или аналога) в том же коммите — SoT растёт, реестр вопросов отстаёт (нарушает rule 11 cross-cutting alignment). Alternative: STOP, добавить/закрыть Q-NN записи в open-questions, только потом commit.
18. Backlog-task, затрагивающий crypto-архитектуру, ушёл в `/speckit.specify` без alignment sweep — устаревшее описание может привести speckit по ложному пути (нарушает rule 11). Alternative: STOP, вызвать `procedure-crypto-alignment-sweep` для этого task'а; если marker `blocked-on-question` — не начинать speckit пока не решится соответствующий Q-NN.
