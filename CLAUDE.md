...
- **Status workflow (4 статуса, обновлено 2026-06-23):**
  ...
  - `Paused` — работа была начата (есть spec.md / частичный код в untracked / в stash / в ветке), но временно отложена. Два под-случая:
    - **(a) Отложена в пользу другой задачи** — note: «причина паузы», «куда положили частичную работу» (stash/ветка/spec-папка).
    - **(b) PR смержен, но не все AC зелёные** (real-device test, OEM verification, doc update остались `[ ]`) — note в `<!-- SECTION:PAUSE_REASON:BEGIN -->`: «PR #X merged YYYY-MM-DD, blocked AC: #N, #M». Снимается, когда оставшиеся AC закрываются.
    Отличается от `Draft` тем, что **что-то уже написано**. Возврат в `In Progress` (случай a) подразумевает recovery работы; переход в `Done` (случай b) подразумевает закрытие оставшихся AC и повторный `pre-pr-backlog-sync`.
  - `Done` — merged в `main` **И** все AC `[x]`. Если merged без зелёных AC → `Paused` (b), не Done.
...
- **Pre-PR sync (HARD RULE для AI, добавлено 2026-06-24):**
  - **Перед `gh pr create`** на любой ветке, привязанной к spec'у или к backlog-task'у, AI **обязан** вызвать skill [`pre-pr-backlog-sync`](.claude/skills/pre-pr-backlog-sync/SKILL.md). Skill:
    ...
    2. Парсит `<!-- AC:BEGIN -->...<!-- AC:END -->`, по каждому AC автоматически проверяет (grep по коду / тестам) + спрашивает владельца про manual gates (real-device, OEM, doc).
    3. Решает статус: **все [x] → Done** (+ Final Summary); **есть [ ] → Paused (b)** с явным указанием blocked AC.
    4. Коммитит изменения в `backlog/tasks/` отдельным коммитом, затем возвращает control для `gh pr create`.
  - PR description **должен** содержать строку `Backlog: task-N → <new status>` (и `pending AC: #X, #Y` если Paused).
...
- **`In Progress → Done` только через `pre-pr-backlog-sync`** при условии, что **все** AC `[x]`. Никакого «merged = Done» автомата.
