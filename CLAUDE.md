...
Fresh AI session или новый collaborator, продолжающий крипто-работу, начинает с [`docs/architecture/crypto.md`](docs/architecture/crypto.md) — там authoritative snapshot (сценарии, glossary, rejected alternatives, топ-7 способов взорвать, Decision index, validation set, post-MVP roadmap). Priority queue = backlog Kanban (`backlog sequence list --plain`). Серверные endpoints и zero-trust baseline — [`docs/architecture/server.md`](docs/architecture/server.md).
