...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 [hand] Все production-классы с версией формата используют точечную строку и несут `minReaderVersion` + `minWriterVersion`; номер ни у одного формата не уменьшился относительно отгруженного — кроме крипто-форматов, вынесенных в TASK-141 по решению владельца
- [x] #2 [hand] Константы версий сведены к трём именам из `wire-format.md` §11, по одной на формат, объявлены рядом с типом; литералов версий в местах вызова не осталось
- [x] #3 [hand] Все JSON-ассеты и фикстуры обновлены; roundtrip- и backward-compat-тесты зелёные
- [x] #4 [hand] Firestore security rules и их тесты обновлены под строковую версию; запрет отката версии сохранён и проверяется тестом на границе 9→10 в обе стороны
- [N/A] #5 [hand] Строгий режим `ArchitectureFitnessTest` — перенесён в TASK-142 вместе с остальным автопилотом; здесь остаётся мягкий режим (версия из именованной константы), он зелёный
- [x] #6 [hand] Разобраны две находки инвентаризации: `theme-catalog.json` / `hint-pool.json` несут заголовок версии, второе дерево bundled-пресетов подтверждено мёртвым и удалено
- [x] #7 [hand] Модуль версий вынесен в `:core:wire` (пакет `family.wire`), остаётся листом без зависимостей; крипта не знает о версиях ничего — проверено `verifyCryptoIsolation` и поиском по `core/crypto` + `core/keys`
<!-- AC:END -->
...
Переведено 18 форматов плюс правила Firestore, воркер push и эталонные фикстуры. Версия из целого числа стала точечной строкой, у каждого формата появились `minReaderVersion` и `minWriterVersion`, читатель получил третий исход — «только чтение».

**Найдено по дороге и починено:**
- Правила Firestore уже требовали строковый заголовок, а адаптер входа писал целое число — **создание учётной записи при входе отклонялось**. Компилятор этого не видел: документы `identity-links` и `users` собираются как ad-hoc мапы, а не из типов. Добавлены два регрессионных теста.
- Набор тестов правил гонялся параллельно, деля один эмулятор, — падения расползались по чужим suite'ам. Прогон сделан последовательным.
- Дублирующая проверка версии в приёмнике push, сравнивавшая `schemaVersion` вопреки §3.
- Замена жёсткого `schemaVersion == 1` в правилах открывала возможность заблокировать общий документ (`minReaderVersion: "999.0"`) без починки с клиента — добавлен потолок `maxAcceptedReaderVersion()`.
- Глоссарий и decision-документ пересказывали правила и разошлись с ними (§12).

**Вынесено в отдельные задачи:**
- TASK-141 — крипто-форматы и переименование `cryptokit` → `family`.
- TASK-142 — автопилот (хук, скилл, генератор, кросс-языковая сверка).
- TASK-143 — QR-ссылка спаривания: формат найден вне инвентаризации, выбор формы имеет цену для пользователя.

**Оставлено намеренно:** `workers/backup` (проверяет крипто-блоб → TASK-141), `KeyBlob` (там же), `WorkerPushSender` конвертирован, но помечен как мёртвый путь — `/notify` никто не обслуживает и вызовов нет.
