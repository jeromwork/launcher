...
- [ ] #11 [hand] Fitness rule `wire-format-hygiene` реализован в `lint-rules/` по образцу существующих детекторов, с тестом
- [ ] #12 [hand] Fitness rule `crypto-flows-clock-hygiene` — запрет `Clock.System.now()` / `System.currentTimeMillis()` в `core/keys/*`, `core/crypto/*`, `app/adapters/openmls/*`, `*.pairing.*` (per audit 2026-07-07 item #2 / CANDIDATE-3)
<!-- AC:END -->
