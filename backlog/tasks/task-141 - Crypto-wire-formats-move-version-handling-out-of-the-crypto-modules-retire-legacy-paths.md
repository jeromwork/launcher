...
**Переименование пакетов — первым коммитом, до всего остального.** `cryptokit.crypto` → `family.crypto`, `cryptokit.keys` → `family.keys`. Корень `family` = семейство продуктов (лаунчер, мессенджер, галерея), **не** целевая аудитория — записать это комментарием в заголовке обоих модулей, слово двусмысленно именно в этом продукте. `:core:push` уже `family.push`, `:core:wire` переведён в TASK-138 (коммит `39e36c1`), эти два — последние.
