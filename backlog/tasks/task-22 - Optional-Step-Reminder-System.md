...
**Planned.** Зависит от TASK-16 (P-1 — `optionalSteps` поле в schema), TASK-17 (P-2 — examples of optional Android steps).
...
ЧТО СТРОИМ:
В preset (schemaVersion 2 из TASK-16) есть optionalSteps. OptionalStepTracker отслеживает completion. В Settings показывает badge «N не настроенных шагов». Tap на badge → список с прогрессом. Tap на step → запускает WizardStep. ЯВНО без push-уведомлений (rule 10 compliance).
...
DEPENDENCIES:
- TASK-16 (P-1 schema v2 с optionalSteps).
