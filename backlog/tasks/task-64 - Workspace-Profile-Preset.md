...
3. Все edges/devices/configs — переживают потерю телефона (recovery через TASK-6/TASK-58).

## Зачем

Это **первая реальная фича**, которую видит конечный пользователь после TASK-57/58/59 foundation. Без TASK-60 foundation существует, но никто не пользуется. С TASK-60 — admin-сценарий («дочка управляет маминым телефоном за 200 км») закрыт и работает.

Также это **проверка правильности foundation**: если для workspace профиля **не потребовалось** ни одной строки Kotlin — значит TASK-57/58/59 действительно generic. Если потребовалось — баг foundation, возвращаемся.
...
- **`workspace.wizard.manifest.json`** — последовательность шагов:
  - Step 1: `wizard.step.google-sign-in` (через `CheckSpec.AuthState` / `ApplySpec.RequestSignIn` из TASK-57).
  - Step 2: `wizard.step.pair-device` (через TASK-59).
...
- Любой Kotlin код. Если AI-агент чувствует «надо бы добавить Kotlin» — **STOP**, это сигнал что TASK-57/58/59 не доделаны, возвращаемся туда.

## Состояние

**Planned.** Закрывается последней в цепочке TASK-57 → TASK-58 → TASK-59 → TASK-60. Финальный deliverable, превращающий foundation в видимую фичу.
...
Бандлится при build, появляется в first-launch picker (TASK-57) как второй вариант.
Wizard: Google sign-in → pair device → готово. Main screen: paired devices dashboard.

ЗАЧЕМ:
Первая видимая фича после foundation TASK-57/58/59. Закрывает сценарий «дочка управляет
...
DEPENDENCIES:
- TASK-57 (Profile Composition Foundation v2) — first-launch picker + sign-in pool entries.
- TASK-59 (Pairing Feature) — pairing pool entries (tile.pairing.list, wizard.step.pair-device).
- TASK-3 (AuthProvider) — Verification, через TASK-57 wizard.step.google-sign-in.
...
- Спарил с simple-launcher устройством (TASK-59 flow) → у workspace в списке появилось → у simple-launcher в settings/pairing-list появилось.
...
- Recovery на новом устройстве workspace профиля → paired devices автоматически восстановлены через TASK-58.
...
- Article VII §13 (no if-profileId): пройдено fitness function из TASK-57.
