...
- **Serialization** = entity-grouped, mirror of Fleks `Snapshot`: `{id, parentId, components:[{type,…}], tags:[…]}`. kotlinx polymorphic (`classDiscriminator="type"`), zero custom serializer. `schemaVersion` present, **value stays 2, no bump, no migrator** (Article XX pre-MVP). Versioning discipline is now owned by [`wire-format.md`](../architecture/wire-format.md); this format converts to the dotted-string form on next touch (TASK-138), never downward.
