...
- **Status**: 🔴 open
- **Context**: сессия 2026-07-02, вопрос «бабушка recovery — увидит ли старую переписку/фото?».
- **Blocks tasks**: TASK-6, TASK-28, TASK-32, TASK-70.
- **Priority**: **critical** (пользовательская ожидаемая функция).
- **Session-tag**: `theme-12-history-backup`.

MLS forward secrecy запрещает расшифровку сообщений до Welcome. WhatsApp обходит это через **local encrypted DB + iCloud/Google Drive backup** (ключ у пользователя). У нас **нет такого backup'а**. Варианты:
- **A**. Копировать WhatsApp: локальная SQLCipher DB → periodic backup encrypted-blob → Google Drive/iCloud/наш R2 → recovery через passphrase.
- **B**. Как Signal — история недоступна после recovery, явно предупреждаем при onboarding'е.
- **C**. Гибрид: history-backup opt-in (не в MVP, добавить позже).

Влияет на MVP UX бабушки и на положение в Θ.4 Тема 9.
...
Связано с Q-09 (если backup истории есть — риск attacker'а получить историю выше).
...
1. **Critical** — Q-09 (history backup) — влияет на MVP UX и на все content-storage задачи.
2. **High**:
...
4. **Low** — Q-05, Q-10, Q-15.

**Рекомендация владельцу**: следующая mentor-сессия — **Q-09** (history backup). Тема имеет прямой пользовательский impact и определяет несколько других задач.
