...
- *Соответствует:* правилам обратной совместимости чтения — [`docs/architecture/wire-format.md`](../architecture/wire-format.md).
...
| 2026-07-08 | **CLAUDE.md rule 13 «Zero-knowledge server posture» принят** (TASK-57, [rule 13](../../CLAUDE.md#L261)). Server sees only opaque blobs + Ed25519 signatures; no ACL graph, no eventType routing, no business logic on plaintext. Refuse patterns 21-28 добавлены. Skill [`checklist-zero-knowledge-server`](../../.claude/skills/checklist-zero-knowledge-server/SKILL.md) enforces posture + server-log accumulator model. Standalone server-research-tasks (TASK-59 recovery vault, TASK-60 push 4KB) закрыты как superseded — их research теперь в [`server-log.md`](server-log.md) Part B Q-1/Q-2, deep validation происходит в момент feature-task'и (TASK-6 recovery, TASK-27 messenger, etc.). Companion к rule 12 (zero-trust posture, TASK-105 baseline). ADR-008 social recovery помечен SUPERSEDED — helper graph переработан на client-coordinated blob. | TASK-57 mentor-сессия 2026-07-08 |
...
## SRV-CONFIG-DEPRECATION: Retire ConfigDocument in favor of Profile-as-source-of-truth (TASK-127 follow-up)

**Контекст**: `ConfigDocument` (spec 008/009 wire format для `/links/{linkId}/config/current`) был MVP-костылём для доставки настроек admin↔managed device. TASK-120 ввёл `Profile` как device-local модель, но оставил `ConfigDocument` для cloud delivery — итог: **два параллельных источника правды** для одних и тех же настроек (плитки, тема, шрифт), связанных ad-hoc маппингом.

TASK-127 (2026-07-15) закрыл gap для HomeScreen: `ProfileBackedFlowRepository` читает `Profile` напрямую через ECS-style Query. `ConfigDocument` больше не используется на HomeScreen пути, но остаётся живым для admin push (spec 009 FCM).

**MVP workaround (сейчас)**: `ConfigDocument` + `ConfigEditor` + `ConfigBackedFlowRepository` остаются в коде для admin push. HomeScreen переехал на `ProfileBackedFlowRepository`. Inline TODO расставлены в местах остаточного использования.

**Own-server destination** (unified model):
- **Profile сам сериализуется в Firestore** через `/links/{linkId}/profile/current` вместо ConfigDocument.
- Admin push = получить Profile, отредактировать, отправить целиком (не diff слоя ConfigDocument).
- `ConfigDocument`, `ConfigEditor`, `ConfigBackedFlowRepository`, `linkId`-based routing для config — удаляются полностью.
- Wire format Profile v3+ становится единственным shared format между device ↔ server ↔ admin device.
- Cost-of-swap: 1 spec (unified profile-sync), новый adapter `RemoteProfileStore`, migration script для существующих ConfigDocument → Profile.

**Inline TODO в TASK-127 code**:
```kotlin
...
**Trigger**: следующая итерация admin push spec'и (либо явная задача Profile-Sync Unification) — когда есть capacity для переработки cloud transport layer + wire format migration writer.

**Source**: TASK-127 Decision block (mentor session 2026-07-15), `docs/architecture/preset-model.md` § "Deprecated: ConfigDocument path".
