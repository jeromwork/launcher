...
| **snow** | snow — готовая библиотека рукопожатия (Noise_XX) |
| **mls** | openmls — готовая библиотека групп (RFC 9420) |
...
- **Список членов комнаты** = **MLS roster** (кто внутри). Заменил старую модель «список гостей / access-grant».
- **Ритуал знакомства** = **handshake** (два устройства впервые встречаются).
- **Личная записная книжка** = **TrustEdge** (мои имена для знакомых).

> Историческая справка: ранее использовались термины `stableId` (заменён на `identity_id`), `access-grant` + `envelope-per-recipient` (упразднены с pivot'ом на MLS group membership).
...
|---|---|---|---|
| **libsodium** (ionspin KMP wrapper) | Готовые крипто-примитивы (шифрование, ключи, пароли) | Signal, WhatsApp, Wire — все | Швейцарский нож крипты |
| **snow** (Rust через UniFFI) | Готовое рукопожатие (Noise_XX) | WireGuard, WhatsApp companion | Ритуал знакомства «под ключ» |
| **openmls** (Rust через UniFFI) | Готовые группы (MLS RFC 9420) | SRLabs-audited 2024, Wire messenger | Комната с автозамками |

> Историческая справка: ранние сессии рассматривали `mls-rs` (AWS Labs) как основной MLS-выбор. Финальный выбор — **openmls** (см. [`docs/architecture/crypto.md`](../architecture/crypto.md)). `mls-rs` остаётся как **exit ramp** (та же RFC 9420 wire format, swap ~3-5 дней через `GroupCryptoPort`).
...
| 15 — Multi-app cohabitation | [TASK-25](../../backlog/tasks/task-25%20-%20Multi-app-Cohabitation-Chain-of-trust-Recovery.md) | Depends on [TASK-112](../../backlog/tasks/task-112%20-%20Decision-Cross-platform-IdentityVault.md) (IdentityVault port boundary, Discussion). Android-first stance. |
