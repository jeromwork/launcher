...
**Known debt (do NOT "fix" in docs — owning task named)**: `family.pairing.*` currently lives **inside `:core:crypto`** and holds `@Serializable` types (`PublicKey`, `SigningPublicKey`, `DeviceId`) + `ByteArrayBase64Serializer`. That violates the crypto-SDK "no serialization" invariant. Target = its own module `:core:pairing` (serialization becomes legal there — pairing is not a crypto primitive). Migration = follow-up task (see [`crypto.md`](crypto.md) zone map). Until migrated, the invariant reads "no serialization in `family.crypto.*` / `family.keys.*` packages".
