...
**Волна 1 — крипто-decisions построенные на foundation**:
...
Приоритетные implementation candidates (по состоянию 2026-07-08):
1. **TASK-16** implementation — wire format discipline + fitness rule. ~1 неделя. Разблокирует правильные formats для всей крипто-work.
2. **TASK-112** implementation — KeyVault port. ~1 неделя mechanical. Foundation для всех key operations.
3. **TASK-2** F-CRYPTO Core — **Done** (openmls через UniFFI integrated).

**Что дальше** (после implementation Волны 0 foundation):
- Feature tasks (TASK-67 pairing, TASK-42 messenger encryption, TASK-11 photo album) активируются в порядке product priority.
- Decision-tasks Волны 2 (TASK-115/116/117) уходят в Session 2 mentor при подходе к messenger spec.
