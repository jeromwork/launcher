...
**Domain ports** (rule 1): `CryptoPort`, `GroupPort`, `KeyPackagePort` (all PLANNED — 0 code, TASK-123). Built ports live in the zone files. `KeyVault` (TASK-112) = **boundary 2 (data-key ops)**, MVP narrowed to the single `exportDerivedKey` hatch + `enum Purpose { MLS_SIGNATURE, NOISE_STATIC }` (AEAD/MAC deferred additively — no consumer today, rule 4); NOT built, adapter lands with first consumer (TASK-67/124). Identity-key ops (`AsymmetricCrypto`) and key-at-rest/capability (`SecureKeyStore`) are separate already-built ports, not part of `KeyVault`. The vault is three boundaries, not one — see [`crypto-key-hierarchy.md`](crypto-key-hierarchy.md) §Key vault (deep-research 2026-07-22).
...
| TASK-112 | KeyVault port boundary | In Progress (decided) | `KeyVault` = boundary 2 of 3, MVP = `exportDerivedKey` hatch + `Purpose{MLS_SIGNATURE,NOISE_STATIC}`; AEAD/MAC deferred (no consumer, rule 4). Identity-key ops + capability = separate built ports. NOT built. |
