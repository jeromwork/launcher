...
| **D — Profile (per-device runtime state)** | Локальное состояние конкретного устройства: `basedOnPreset` (ref на activated preset), `components: List<ProfileComponent>` (с `status ∈ {Pending, Applied, Failed, Skipped}`), `preWizardSnapshot` (для Undo), `state: ProfileState` (opaque holder для `CapabilityQuery`). НЕ shareable — содержит device-specific bindings. Живёт в DataStore. | `com.launcher.preset.model.Profile` + `com.launcher.preset.port.ProfileStore` + `DataStoreProfileStore` | TASK-120 foundation; расширяется в TASK-70 (multi-profile для admin app) |
...
| **`profile`** | 2 | Per-device state — `basedOnPreset` + resolved `components: List<ProfileComponent>` + `preWizardSnapshot` + `state: ProfileState` (opaque). НЕ shareable, живёт в DataStore. | `ProfileStore` port | DataStore (`task120_profile.preferences_pb`), не файл |
