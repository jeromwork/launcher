...
> - **`Profile`** wire format schemaVersion=2 хранит `entities: List<Entity>` (ECS-мир; free-bag компонентов + теги, apply-state = `LifecycleState` компонент). Полная модель — **[`docs/architecture/ecs.md`](../architecture/ecs.md)** (single source of truth).
>
...
| **B — Pool (реестр declarations)** | Единый каталог параметризованных `ComponentDeclaration`-записей: `id` + `component` + `wizardBehavior` + `critical` + `descriptionKey`. Заменяет разбиение по kind (`system-settings.pool` / `ui-customization.pool` / `tile.set`). Загружается через `PoolSource` port. Bundled seed — `assets/preset/pool.json` schemaVersion=1. | `com.launcher.preset.model.Pool` + `com.launcher.preset.port.PoolSource` + `BundledPoolSource` | TASK-120 foundation |
| **C — Preset (shareable composition)** | JSON-шаблон целого варианта устройства с **тремя ортогональными полями**: `wizardFlow[]` (порядок first-run), `settingsMap[]` (категории для Settings UI), `activeComponents[]` (source of truth для reconcile). schemaVersion=2. Пример: `simple-launcher`, `launcher`, `workspace`. Идентити-free, shareable per rule 9. | `com.launcher.preset.model.Preset` + `assets/preset/bundled-presets/*.json` + `BundledPresetSource` | TASK-120 foundation; presets ship'ятся как JSON, не как Kotlin-код (rule 4 MVA + Article VII §13) |
...
|---|---|---|---|---|
| **`pool`** | 1 | Каталог `ComponentDeclaration`-записей. Единый реестр вместо разбиения по kind. | `PoolSource` port | `app/src/main/assets/preset/pool.json` |
| **`preset`** | 2 | Shareable композиция: `wizardFlow[]` + `settingsMap[]` + `activeComponents[]` + `layoutKey`. Три поля потребляются разными consumer'ами (Wizard / Settings UI / ReconcileEngine). | `PresetSource` port | `app/src/main/assets/preset/bundled-presets/{simple-launcher,launcher,workspace}.json` |
| **`profile`** | 2 | Per-device ECS state — `basedOnPreset` + `entities: List<Entity>` (free-bag компоненты + теги; apply-state = `LifecycleState` компонент) + `preWizardSnapshot` + `state: ProfileState`. НЕ shareable, живёт в DataStore. Модель → [`ecs.md`](../architecture/ecs.md). | `ProfileStore` port | DataStore (`task120_profile.preferences_pb`), не файл |

**Эволюция**: любое изменение — через `schemaVersion` bump + migration writer + backward-compat read test (CLAUDE.md rule 5). Fitness function #10 запрещает literal-strings в user-facing полях (`labelKey`, `descriptionKey`, `wizardTitleKey`, `categoryKey`, `layoutKey`, `iconKey`) — только dotted i18n keys, резолвятся через `LocalizedResources` port.
