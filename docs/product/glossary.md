...
> - **`Preset`** wire format wire format `"2.0"` получает **три поля**: `wizardFlow` / `settingsMap` / `activeComponents` (заменяет single `steps[]` list).
> - **`Profile`** wire format `"2.0"` хранит `entities: List<Entity>` (ECS-мир; free-bag компонентов + теги, apply-state = `LifecycleState` компонент). Полная модель — **[`docs/architecture/ecs.md`](../architecture/ecs.md)** (single source of truth).
>
...
## 2. Четыре слоя архитектуры (TASK-120 rewrite 2026-07-11)

Модель фундамента после TASK-120 — **Component / Pool / Preset / Profile**. Заменяет прежнюю Step/StepType/pool-per-kind модель (см. §8 migration table).

| Слой | Что | Где живёт в коде | Когда строится |
|---|---|---|---|
| **A — Component** | Sealed hierarchy декларативных «настраиваемых сущностей» — `AppTile`, `FontSize`, `Sos`, `Toolbar` (MVP wave). Каждый subtype — pure-Kotlin `@Serializable` data class, полиморфный по `type` discriminator. Идентити-free per rule 9 (identity-bound значения резолвятся `Provider` через отдельные ports типа `PairingService`). | `core/src/commonMain/kotlin/com/launcher/preset/model/Component.kt` | TASK-120 foundation; расширение подтипов — task-121 (MessengerTile), draft-1 (SignInGoogle) |
| **B — Pool (реестр declarations)** | Единый каталог параметризованных `ComponentDeclaration`-записей: `id` + `component` + `wizardBehavior` + `critical` + `descriptionKey`. Заменяет разбиение по kind (`system-settings.pool` / `ui-customization.pool` / `tile.set`). Загружается через `PoolSource` port. Bundled seed — `assets/preset/pool.json` (поставляемый файл объявляет `"1.0"` — читатель 2.0 понимает его полностью). | `com.launcher.preset.model.Pool` + `com.launcher.preset.port.PoolSource` + `BundledPoolSource` | TASK-120 foundation |
| **C — Preset (shareable composition)** | JSON-шаблон целого варианта устройства с **тремя ортогональными полями**: `wizardFlow[]` (порядок first-run), `settingsMap[]` (категории для Settings UI), `activeComponents[]` (source of truth для reconcile). Версия `"2.0"`. Пример: `simple-launcher`, `launcher`, `workspace`. Идентити-free, shareable per rule 9. | `com.launcher.preset.model.Preset` + `assets/preset/bundled-presets/*.json` + `BundledPresetSource` | TASK-120 foundation; presets ship'ятся как JSON, не как Kotlin-код (rule 4 MVA + Article VII §13) |
| **D — Profile (per-device runtime state)** | Локальное состояние устройства (ECS-мир): `basedOnPreset` (ref на пресет), `entities: List<Entity>` — свободный мешок компонентов + теги; apply-state = `LifecycleState` компонент (`Pending`/`Applied`/`Failed`/`Skipped`/`Unverifiable`), **не** поле `status`; `preWizardSnapshot` (Undo), `state: ProfileState` (opaque). Самодостаточен для поведения+home; презентация — в пресете (I1-I3). НЕ shareable. Полная модель → **[`ecs.md`](../architecture/ecs.md)**. | `com.launcher.preset.model.Profile` + `ProfileStore` + `DataStoreProfileStore` | ecs.md (TASK-136 canonical ECS); расширяется в TASK-70 |

**Единый движок настройки**: `ReconcileEngine.run(RunMode)` покрывает все 4 сценария применения — `Wizard` (первый запуск), `BootCheck` (реapply критичных при загрузке), `Single` (правка одного Component из Settings), `RemotePush` (admin меняет активные компоненты). Диспатч — через `ProviderRegistry.resolve(component)` с 3-tier fallback (vendor → platform → NoOp). Fitness function #2 запрещает `when(component) { is Component.X ... }` в engine — только через registry.

**Wizard = view Preset'а**, не отдельная сущность (Article VII §11): UI-слой поверх `ReconcileEngine.run(RunMode.Wizard)`, получает Interactive-шаги через `InteractionSink`.

**Принцип «preset = данные, не код»** сохраняется (Article VII §13, Project-Specific Direction §3): новый preset ship'ается как новый JSON в `assets/preset/bundled-presets/`, **не** как Gradle-модуль и **не** как `if (presetId == "x")` ветка. Новый Component subtype ship'ается как одна sealed data class + один `Provider<T>` adapter + строчка в Koin-модуле — fitness function #3 verifies coverage.

---

## 3. Wire-format семейство после TASK-120

После TASK-120 rescope семейство bundled JSON-схем сжимается до **трёх** kinds. Прежние 5 (`wizard.manifest`, `screen.layout`, `tile.set`, `system-settings.pool`, `ui-customization.pool`) поглощены новой моделью — см. §8 migration table.

| Имя схемы | schemaVersion | Описывает | Загружается через | Bundled location |
|---|---|---|---|---|
| **`pool`** | `"2.0"` | Каталог `ComponentDeclaration`-записей. Единый реестр вместо разбиения по kind. | `PoolSource` port | `app/src/main/assets/preset/pool.json` |
| **`preset`** | `"2.0"` | Shareable композиция: `wizardFlow[]` + `settingsMap[]` + `activeComponents[]` + `layoutKey`. Три поля потребляются разными consumer'ами (Wizard / Settings UI / ReconcileEngine). | `PresetSource` port | `app/src/main/assets/preset/bundled-presets/{simple-launcher,launcher,workspace}.json` |
| **`profile`** | `"2.0"` | Per-device ECS state — `basedOnPreset` + `entities: List<Entity>` (free-bag компоненты + теги; apply-state = `LifecycleState` компонент) + `preWizardSnapshot` + `state: ProfileState`. НЕ shareable, живёт в DataStore. Модель → [`ecs.md`](../architecture/ecs.md). | `ProfileStore` port | DataStore (`task120_profile.preferences_pb`), не файл |

**Эволюция**: правила подъёма версии и требуемые тесты — [`wire-format.md`](../architecture/wire-format.md) §5 и §11 (не пересказывать здесь, §12). Fitness function #10 запрещает literal-strings в user-facing полях (`labelKey`, `descriptionKey`, `wizardTitleKey`, `categoryKey`, `layoutKey`, `iconKey`) — только dotted i18n keys, резолвятся через `LocalizedResources` port.

**Additive-only рост pool.json** (см. contracts/pool.md): добавление declaration / нового subtype / нового optional поля с default — OK. Удаление / переименование `id` / смена `component.type` существующей записи — forbidden (breaking read). Fitness function #8 предотвращает разрастание: warn при ≥ 4-5 declarations per Component subtype, error при ≥ 6 — использовать `paramsOverride` в preset вместо новой записи.

**Что НЕ является отдельной схемой** (rule 4 MVA):

- ❌ `theme` — hardcoded поле внутри Component variant (например `Toolbar.layoutKey`) или отдельный будущий `Component.Theme` subtype, добавится при появлении конкретного use-case (marketplace тем / admin-remote-recolor).
- ❌ `hint.set` — runtime-подсказки post-wizard. Отдельный kind появится когда первый hint реально нужен.
- ❌ `permission.set` — это Component variant (например `Component.SystemPermission`), не отдельный документ. MVP wave не включает — добавляется в draft-1 если нужно.
- ❌ separate `wizard.manifest` — поглощён `Preset.wizardFlow[]` полем.
- ❌ separate `system-settings.pool` — записи с `component.type: SystemSetting` в общем `pool`.
- ❌ separate `ui-customization.pool` — записи с `component.type: FontSize / Toolbar / ...` в общем `pool`.
...
| Поле | Тип | Назначение |
|---|---|---|
| `schemaVersion` | — | Версия схемы. Форма и правила — [`wire-format.md`](../architecture/wire-format.md) |
...
**Правила живут в [`wire-format.md`](../architecture/wire-format.md), здесь только ссылка** — тот файл выигрывает при любом расхождении (§12).

Прежний текст этого раздела пересказывал дисциплину и к TASK-138 стал неверным: он утверждал, что читатель отказывается, встретив `schemaVersion` выше знакомого. Теперь у документа **три** поля версии, и `schemaVersion` — диагностика, на которой читатель не принимает **никаких** решений (§3). Отказ — по `minReaderVersion`; между «читаю» и «отказываюсь» появился третий исход, **только чтение**.
...
| «ConfigDocument» (спека 008) | **deprecated** как runtime-конфиг — заменён Profile-as-source-of-truth (см. [`ecs.md`](../architecture/ecs.md) §7). Облачная единица (что летит на сервер) — открытый вопрос TASK-70. |
