...
- Roundtrip tests для все wire-формат изменений.
...
- Steps composition:
  - Welcome screen (большой шрифт, простой язык).
  - Language detection (system locale).
  - Text size selection (default scaled +1.2× system).
  - Theme selection (warm-light / warm-dark).
  - Grid preset selection (3 options).
  - Permissions: ROLE_HOME (deep-link `Settings.ACTION_HOME_SETTINGS`), POST_NOTIFICATIONS (Android 13+).
  - Pairing step (skippable — «настрою позже»).
  - Config template picker (3 starter templates).
...
Отдельное приложение (или shared module), на Jitsi Meet. SSO с launcher через F-4 AuthProvider (one Google login для обоих apps). Simplified UX (extremely large buttons, easy answer flow). Group call invites через Family Group.

**Closes**: D-23 implementation (MVP — handoff; post-MVP — separate Jitsi app + SSO).
