...
| 010 | `setup-assistant-soft-checks` | **Code-complete 2026-05-19** (branch `010-setup-assistant`, Phase 0-7 готовы — preflight + ARCH-016 closure + wizard + GMS hard-block + call confirmation + Settings soft-checks engine + paired devices local-first revocation + 7-tap challenge gate). Phase 8 verification artefacts (smoke / perf checkpoints + senior-safe walkthrough doc) — готовы; physical-device smoke (T052/T053/T065/T093/T102/T105/T106 + macrobench T107 + APK delta T108) deferred per memory `feedback_critical_mentor_stance.md` / `reference_testing_environment.md`. ARCH-016 закрыт. Ready for `/ultrareview` + PR | 006 (параллельно 007/008) |
...
### Spec 011 — `e2e-crypto-foundation` *(переименован 2026-05-22; ранее `contacts-and-e2e-encrypted-media`)*

**Статус:** в разработке (ветка `011-contacts-and-e2e-encrypted-media`, переименование артефактов в процессе).

**Зависит от:** 006 (IconStorage namespace `private:`), 007 (pairing), 008 (`Contact.photoRef` reserved field).

**Контекст переноса (2026-05-22):** в mentor-сессии решено **разрезать** спек надвое. Изначально 011 объединял «крипто-машину» и «фото контактов». В новой формулировке:
- **011 = только криптофундамент** — porty, real-adapters, fake-adapters, envelope wire-format, pairing extension (Pub-key publication), Storage adapter, reference counting, cleanup. **БЕЗ visible feature** (никаких фото на плитках).
- **012 = первый клиент фундамента** — фото контактов и личных документов, UI, Contacts Picker integration. Использует 011.

Это **архитектурное решение, не локальная правка** — фундамент 011 универсален и используется **всеми** будущими спеками, где нужна криптография (фото, двусторонние пары, группы, multi-device, key rotation, Jitsi room access, vendor integrations, hardware integrations).

**Что делает 011 (только фундамент):**
- **Per-device asymmetric keys** (X25519) — каждое устройство при первом запуске генерирует свою пару (Pub, Priv). Priv хранится в Android Keystore (через AES-wrap, потому что Keystore не умеет X25519 нативно). Pub публикуется в Firestore.
- **Digital signature keys** (Ed25519) — отдельная пара для signing (auth Pub-ключа в Firestore, будущий Jitsi room auth, будущая vendor authentication).
- **Encryption** (XChaCha20-Poly1305 AEAD via libsodium) — hybrid encryption: blob шифруется одноразовым CEK, CEK seal'ится через `crypto_box_seal` для каждого получателя.
- **Hashing** (BLAKE2b via libsodium) — для дедупликации blob'ов, integrity checks, будущих fingerprint'ов.
- **Membership-agnostic envelope format** — массив `recipients` произвольной длины (в 011 длина 1, в будущих спеках N).
- **Storage adapter** для зашифрованных blob'ов (Firebase Storage, заменяется на свой backend по `SRV-CRYPTO-001`).
- **Reference counting + cleanup** — `BlobReferenceLedger` SQLite, `LinkRegistry.revoke()` расширение, `Link.KNOWN_SUBCOLLECTIONS` += `private-media`.
- **e2e smoke-тест** на синтетическом blob (16 байт) между двумя устройствами — gate перед merge (компенсирует отсутствие visible feature).

**Что НЕ делает 011 (out-of-scope для всего фундамента):**
- Visible feature — никаких фото на плитках, никакого UI, никакого Contacts Picker (это спек 012).
- Realtime stream encryption (SRTP/SFrame для audio/video звонков) — это **никогда** не наш envelope; делается внутри Jitsi/SFrame в будущем спеке Jitsi-integration.
- Post-quantum cryptography — отдельный future спек (~020+), libsodium со временем добавит.
- X.509 PKI — отдельный adapter, если когда-нибудь потребуется для vendor integration.
...
|---|---|---|
| spec 012 (фото) | EncryptedMediaStorage, AeadCipher, AsymmetricCrypto, RecipientResolver | UI, Contacts Picker integration, PrivateMediaResolver |
...
| TBD-Audio/Video messages (asynchronous, как WhatsApp voice) | EncryptedMediaStorage + AeadCipher (тот же envelope, другой `metadata.kind`) | UI для записи / проигрывания |
...
| TBD-Hardware integration | DigitalSignature (attestation), HashFunction (integrity) | Hardware-specific протокол |

**Эволюция модели доверия (всё на одном фундаменте, без перешифровки blob'ов):**
1. **011-012**: 1↔1 (admin → managed, одностороннее).
2. **013**: 1↔1 двустороннее (mutual auth).
3. **014**: 1↔N (групповые ключи, семейный круг).
4. **015**: M↔N (multi-device одного владельца, общий identity).
5. **016**: key rotation поверх любого из выше (1↔1, группа, multi-device).

Envelope формат **не меняется** при переходе между моделями — меняется только содержимое `recipients[]` и логика resolver'а. Это главная архитектурная инвестиция 011.

**Server-roadmap:** `SRV-CRYPTO-001` (см. [server-roadmap.md](../dev/server-roadmap.md)) — универсальный маршрут переезда крипто-инфраструктуры на собственный backend, **не привязан к Firebase лимитам**. Триггер — запуск собственного backend проекта (планируется после спека ~35).

**One-way door:** выбор libsodium через Lazysodium-android. Exit ramp = `schemaVersion` + `cipherSuiteId` в envelope с первого commit'a (старые blob'ы продолжают читаться при смене библиотеки). Все остальные крипто-решения проходят через порты — swap библиотеки = swap одного адаптера.

---

### Spec 012 — `contact-photos-and-private-documents` *(добавлен 2026-05-22; выделен из 011)*

**Статус:** stub (см. `specs/012-contact-photos-and-private-documents/spec.md`).

**Зависит от:** 011 (e2e-crypto-foundation), 008 (`Contact.photoRef`), 009 (Contacts Picker).

**Контекст переноса (2026-05-22):** изначально часть спека 011. Вынесено отдельным спеком в mentor-сессии — чтобы 011 был чистым крипто-фундаментом без visible feature, а 012 был **первым клиентом** этого фундамента с реальным UX-выходом для бабушки.

**Что делает:**
- **Фото контактов на плитке у бабушки** — admin добавляет внучку через Contacts Picker (спек 009), Android отдаёт фото из контактной книги, фото шифруется на устройстве admin'а через фундамент 011, грузится в Firebase Storage зашифрованным, у бабушки появляется на плитке настоящая фотография вместо серого квадратика.
- **Фото личных документов** — admin добавляет к раскладке бабушки плитки с её личными документами (паспорт, СНИЛС, медкарта). Бабушка тапает по плитке «Паспорт» → видит фото в полноэкранном просмотрщике.
- **PrivateMediaResolver** — IconStorage namespace dispatch для `private:` (реализация порта из спека 006).
- **Honest reporting на ошибках** — `PartialReason.MediaDecryptFailed` впервые реально эмитится при ошибках расшифровки, admin UI показывает индикатор.

**Что НЕ входит:**
- Шифрование, ключи, envelope, Storage adapter — всё это уже в 011.
- Аудио/видеосообщения «как WhatsApp voice» — отдельный future спек, использует тот же envelope из 011.

**Зачем отдельный спек:** 011 без 012 — это «фундамент без дома», но фундамент **универсальный** и используется не только для фото (см. таблицу в 011). Разделение чистое: 011 = что под капотом, 012 = первый видимый продукт. Дальше другие спеки строят на том же фундаменте свои дома.

---

### Spec 013 — `symmetric-pairing-bidirectional-control` *(перенумерован 2026-05-22; ранее 015)*

См. оригинальный контекст (выделен из 011 в clarify-сессии 2026-05-21) ниже в секции «Spec 013 (был 015)».

**Зависит от:** 011 (per-device key pairs + e2e infrastructure уже введены), 012 (visible UX модели, на котором двусторонний режим строится).

---

### Spec 014 — `family-group-shared-encryption` *(перенумерован 2026-05-22; ранее 016)*

См. оригинальный контекст ниже в секции «Spec 014 (был 016)».

**Зависит от:** 011 (envelope `recipients` массив, `RecipientResolver` интерфейс), 013 (mutual auth).

---

### Spec 015 — `multi-device-recovery` *(перенумерован 2026-05-22; ранее 017)*

См. оригинальный контекст ниже в секции «Spec 015 (был 017)».

**Зависит от:** 011 (per-device key pairs).

---

### Spec 016 — `key-rotation-forward-secrecy` *(перенумерован 2026-05-22; ранее 018)*

См. оригинальный контекст ниже в секции «Spec 016 (был 018)».

**Зависит от:** 011 (envelope `cipherSuiteId` + per-device keys), возможно 014 (для group key rotation).

---

### Spec 017 — `balance-check-and-low-balance-warnings` *(перенумерован 2026-05-22; ранее 012; добавлен 2026-05-09)*
...
### Spec 018 — `offline-detection-and-emergency-reachability` *(перенумерован 2026-05-22; ранее 013; добавлен 2026-05-09)*
...
### Spec 019 — `onboarding-and-tutorials` *(перенумерован 2026-05-22; ранее 014; добавлен 2026-05-19, вынесено из 010)*
...
### Spec 013 (был 015) — `symmetric-pairing-bidirectional-control` *(добавлен 2026-05-21, выделен из 011; перенумерован 2026-05-22)*

**Статус:** backlog (без stub-файла).

**Зависит от:** 011 (per-device key pairs + e2e infrastructure уже введены), 012 (visible model on which bidirectional mode builds).

**Контекст переноса (2026-05-21):** в clarify-сессии спека 011 (см. [specs/011-contacts-and-e2e-encrypted-media/spec.md](../../specs/011-contacts-and-e2e-encrypted-media/spec.md)) обсуждалось «два телефона равноценны, оба могут управлять друг другом». В 011 решено оставить одностороннюю модель из спеков 7-9, а двусторонний pairing вынести отдельным спеком. Crypto-инфраструктура 011 уже membership-agnostic — этот спек добавляет только management-permission и UX.

**Что делает:**
- После `consent.allow` при pairing'е оба устройства получают возможность стать управляющими **по взаимному согласию** (UI «разреши мне видеть твою раскладку»).
- Firestore Security Rules расширяются: каждый member пары имеет permission писать в **обе** субколлекции `/config` (свою и партнёра) при наличии симметричного consent.
- Crypto-протокол остаётся прежним из 011 — пара устройств уже знает Pub-ключи друг друга, шифровка идёт через `RecipientResolver`, который возвращает «другого члена пары».

**Что НЕ входит:**
- Групповые ключи (3+ участников) — спек ~016.
- Multi-device одного владельца (телефон+планшет) — спек ~017.

**One-way door:** UX «бабушка вдруг управляет внуком» может конфузить — privacy-сценарии нужно продумать (что внук не хочет показывать бабушке). Exit ramp — двусторонний режим opt-in, не дефолтный.

---

### Spec 014 (был 016) — `family-group-shared-encryption` *(добавлен 2026-05-21, выделен из 011; перенумерован 2026-05-22)*

**Статус:** backlog (без stub-файла).

**Зависит от:** 011 (envelope с `recipients` массивом, `RecipientResolver` интерфейс).

**Контекст переноса (2026-05-21):** в clarify-сессии 011 обсуждалось «семейный круг как группа в WhatsApp». В 011 решено заложить crypto-инфраструктуру membership-agnostic (envelope `recipients` — массив произвольной длины, see [spec 011 C-2](../../specs/011-contacts-and-e2e-encrypted-media/spec.md)), но саму группу реализовать отдельным спеком.

**Что делает:**
- Понятие «семейная группа» (`familyGroupId`) — N≥3 устройств, объединённых общим членством.
- Любое зашифрованное медиа в группе грузится один раз — envelope содержит `recipients` для всех N членов; каждый расшифровывает своим Priv-ключом.
- Управление членством: добавление через приглашение существующего члена; удаление с key rotation (см. ~018).
- `GroupRecipientResolver` — вторая реализация интерфейса из 011.

**Что НЕ входит:**
- Forward secrecy / периодическая key rotation внутри группы — спек ~018.
- Множество групп для одного устройства (overlap'ы) — на момент 016 каждое устройство в максимум одной группе.

**One-way door:** управление членством (кто может приглашать, удалять, как разрешаются конфликты) — UX-сложность. Exit ramp — стартовая модель «admin-owned group, остальные read-only-members»; richer permissions позже.

---

### Spec 015 (был 017) — `multi-device-recovery` *(добавлен 2026-05-21, выделен из 011; перенумерован 2026-05-22)*

**Статус:** backlog (без stub-файла).

**Зависит от:** 011 (per-device key pairs).

**Контекст переноса (2026-05-21):** в clarify-сессии 011 обсуждался сценарий «бабушка потеряла телефон». В 011 решено: в этой версии при потере telefona без revoke медиа становятся недоступны (зашифровано на потерянное устройство). Решение проблемы — multi-device recovery — отдельный спек.

**Что делает:**
- Понятие «мои устройства» (`ownerId`) — несколько физических устройств одного владельца имеют общий identity.
- Когда внук шифрует медиа для бабушки — envelope содержит `recipients` для **всех** её устройств (телефон + планшет + future replacement device).
- Восстановление при потере: бабушка получает новый телефон, делает re-pairing с внуком, новое устройство добавляется в `ownerId`; для **новых** медиа envelope содержит Pub_new-device; для **старых** медиа применяется одна из exit ramp стратегий:
  - **Re-encrypt:** внук вручную перешифровывает старые медиа для нового устройства (best-effort).
  - **Owner-key escrow:** опциональный «личный ключ владельца» в Android Backup Service, восстанавливаемый при re-installation (см. discussion в 011).
- `MyDevicesRecipientResolver` — третья реализация интерфейса.

**Что НЕ входит:**
- Server-side key escrow (это противоречит e2e).
- Восстановление **без** одного из «моих» оставшихся устройств (если все устройства потеряны одновременно — данные мертвы; принимаем).

**One-way door:** Owner-key escrow в Android Backup Service зависит от Google. При переезде на свой сервер механизм надо менять. Exit ramp — паттерн «Shamir secret sharing across trusted contacts» (см. discussion в 011 «социальное восстановление»).

---

### Spec 016 (был 018) — `key-rotation-forward-secrecy` *(добавлен 2026-05-21, выделен из 011; перенумерован 2026-05-22)*

**Статус:** backlog (без stub-файла).

**Зависит от:** 011 (envelope с `cipherSuiteId` + per-device keys), возможно ~016 (для group key rotation).

**Контекст переноса (2026-05-21):** в clarify-сессии 011 пользователь поднял вопрос «компрометация одного устройства = компрометация цепи». В 011 решено принять текущую модель без forward secrecy и периодической ротации; усиление защиты — отдельным спеком.

**Что делает:**
- **Periodic key rotation:** каждые N дней (или по событию) устройство генерирует новую (Pub_new, Priv_new); старый ключ помечается как deprecated, но не удаляется (для расшифровки старых blob'ов); новые blob'ы шифруются новым ключом.
- **Compromise response:** при подозрении на компрометацию устройства — manual key rotation + sweep всех blob'ов для перешифровки (или принятие, что старые скомпрометированы, новые защищены).
- **Forward secrecy:** новые blob'ы криптографически неподатливы расшифровке даже при последующей компрометации устройства (требует ephemeral keys, например Signal Protocol's Double Ratchet или X3DH).
- Все three механизма не входят в 011 — там single-key per device.

**Что НЕ входит:**
- Post-quantum cryptography — отдельный future спек (~020+).

**One-way door:** ratchet protocols (Signal-style) — серьёзная сложность реализации. Exit ramp — стартовая модель «periodic rotation без ratchet», позже добавление ratchet с миграцией (envelope schemaVersion bump).
