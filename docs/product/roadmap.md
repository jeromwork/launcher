...
Phase 1: Foundation (~5-7 weeks sequential)
   ...
   Шаг 4a: F-5c Push-trigger foundation + config-updated event (📋 PLANNED 2026-06-20, rescoped 2026-06-20 evening)
          → **Generic reusable push infrastructure** (не узкая мини-спека) + первый use case (config-updated). Foundation:
            ...
            - Rate-limit per UID per eventType (защита от abuse, не от UX — нет debounce, push = explicit user action; модель унаследована от retired spec 008, подтверждена в S-8).
            ...
            - PushPayload breaking change: `linkId` → nullable, `ownerUid + configName + eventType` first-class (legacy `linkId` сохранён для backward-compat parse — единственный исторический consumer был retired spec 008).
   ...
   Шаг 4b: Spec 008 RETIRED (🗑️ 2026-06-22)
          → Документация спеки удалена целиком — старая модель (anonymous pair + общий `/links/{linkId}/config/current`) полностью замещена решениями 2026-05-30 (F-4 per-device Google UID) и 2026-06-15 (deferred cloud, envelope E2E через F-5b, FCM payload через F-5c). Концепции collaborative editing / merge UI / pending-changes / config history распределены по будущим спекам Phase 2-4: S-8 (VersionedConfigViewer + Editor — merge UI и history), P-5 (Config Copy Between Own Devices — own-device sync), V-6 (Caregiver Remote Invite — cross-UID admin editing). Legacy Android-код (DefaultConfigEditor, FirebaseConfigApplier, ConfigSyncWorker, LocalConfigStore-SQLDelight, LinkRegistry) остаётся в репо как functional baseline до выполнения соответствующих S/P/V-спек — никакого «спека 008 rewrite» не будет.
