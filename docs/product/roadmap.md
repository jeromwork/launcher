...
- **Config history + rollback** *(добавлено 2026-05-14 из 008 Q7 clarify)*:
  - Subcollection `/links/{linkId}/config/history/{autoId}` — при каждом успешном push в `/config/current` снапшот предыдущей версии переезжает туда.
  - Retention: **последние 10 версий**; 11-я при следующем push'е вытесняет самую старую (housekeeping или TTL).
  - UI: список истории, предпросмотр содержимого версии, кнопка «откатить к этой версии» (фактически — новый push с содержимым выбранной версии, со стандартным conflict-check через optimistic concurrency 008).
...
- iOS управление.
- Сложные retention policies для history (по дате, по размеру) — только «последние 10»; при необходимости расширить позже.
