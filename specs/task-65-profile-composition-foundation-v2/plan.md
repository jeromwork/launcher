...
| OEM-specific deep-link to ROLE_HOME settings (Xiaomi MIUI) | Medium | Fallback toast в ApplySpec.SettingsDeepLink. `// TODO(physical-device): Xiaomi 11T verification`. |
...
| Existing `ProfileSnapshot` тип (см. spec context) — что с ним | Low | Decision R2 в research.md: rename to `ResolvedPresetSnapshot` если используется; delete если dead. Inventory grep в implementation. |
