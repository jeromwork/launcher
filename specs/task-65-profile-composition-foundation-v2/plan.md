...
| `wizard.manifest` (bumped) | `core/src/androidMain/assets/wizard/wizard-manifests/*.json` | existing + migration writer для `presetId` removal | 2 (bumped from 1) |
| `*.pool.json` per-pool | hardcoded в HardcodedPoolSource OR JSON в assets | [`../../../docs/architecture/pool-naming.md`](../../../docs/architecture/pool-naming.md) | per-pool |
...
- `PresetIdBranchingDetectorTest` — positive case (`if (presetId == "x")` в `app/`) → ISSUE; negative case (same в `core/presets/`) → no ISSUE; `when (presetId)` legacy term → ISSUE.
...
| Pre-TASK-65 wizard.manifest без `presetId` field в production у некоторых users (malformed migration trigger) | Medium | Migration writer checks both: `wizardDone == true && activePresetRef == null` (FR-015). Idempotent. Unit test покрывает legacy fixture. |
...
**PASS**. New ConfigKind `preset` added per §10 evolution rule (justified — existing 5 kinds insufficient for shareable preset composition). Per-preset `requiredModules`/`optionalModules` per §8. `wizard.manifest` schemaVersion bump for `presetId` removal — migration writer ships in same commit (rule 5). Article VII §13 enforced via Detekt rule `PresetIdBranchingDetector`. Article VII §14 reused: `WizardEngine.computePending` is config-check master pattern; no per-refId handlers introduced. **Note**: Amendment 1.11 (naming inversion) applied; per-section rewording deferred per «do not preemptively migrate».
...
**PASS**. Contract tests + fakes + integration tests + fitness functions enumerated в §6. Roundtrip tests cover composite PresetRef key (R3 decision). Backward-compat test для presetId removal (R6). Edge case tests for callback throws (CHK011).
...
4. **Persistence**: `PreferencesProfileStore` (DataStore Preferences + JSON-string serialization). Migration writer для `presetId` removal in `wizard.manifest`. Pre-TASK-65 fixture.
...
5. **Migration writer для `presetId` removal** — scoped функция, не разбросанный `if (version == 1)`. Pre-TASK-65 fixture в тестах. (R6)
...
- `../../../docs/architecture/pool-naming.md` — правила именования pool entries.
