...
- [ ] **T413** — T072 smoke-check (закрытие spec 003) на двух эмуляторах per `docs/dev/emulators.md`: full-path FirstLaunch → preset workspace → Home → Flow → tile → confirmation → handoff (mock) → return → Settings → preset switch → simple-launcher → recreate → проверка `!`-индикатора (если применимо к этому spec'у). Параллельно на втором эмуляторе — пресет simple-launcher, тот же путь. Документировать результаты в `specs/003-ui-skeleton/tasks.md` (T072 ✓) и в change-log: «UI migrated to Compose Multiplatform per ADR-005 / spec 004».
