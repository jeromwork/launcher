...
- **FR-057 (Discard confirmation)**: Действие «отменить локальные изменения» (FR-047) MUST требовать confirmation dialog с текстом вида «Удалить несохранённые изменения для этого телефона? Это действие нельзя отменить.». Обоснование (ux-quality checklist 2026-05-14): pending changes могут представлять часы работы пользователя; в отличие от push (который reversible через новую запись), discard уничтожает state без возможности восстановления. Push же НЕ требует confirmation (FR-040, FR-015 — push reversible). Cancel merge UI НЕ требует confirmation (FR-055 — pending preserved).
