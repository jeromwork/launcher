...
**Why this priority**: критично для production-надёжности, но не блокирует P1-P4 (те могут работать с in-memory storage). Без P5 после process kill пользователь Managed-телефона увидит «загрузка» или mock-экран — приемлемо для MVP.
...
- **FR-015 (Push UX feedback)**: При инициации push editor-UI MUST мгновенно показать спиннер «в процессе» рядом с целевым устройством в списке (SC-001). При успешной записи в Firestore (`/config/current` write acknowledged) спиннер сменяется на значок с user-visible надписью «Отправлено ✓». Если за 5 секунд write не подтверждён + `ConnectivityManager.NetworkCallback` сообщает «нет сети» — текст индикатора расширяется «Нет интернета, попробуем позже». Дополнительно — второй значок с надписью «Применено на телефоне ✓» появляется когда Managed обновил `/state.appliedConfigUpdatedAt` (SC-001b). Все user-facing строки нейтральные — без persona-specific терминов (никаких «бабушка», «admin», «сервер»). Внутренние термины (`push`, `Firestore`, `Managed`) — только в коде и в этой спецификации, не в UI-надписях.
...
- **FR-046**: Если в local store есть pending для какого-либо привязанного устройства, **главный экран** editor-приложения MUST показывать **визуальный маркер** с user-visible надписью «Не отправлено» (либо icon-only с описательной a11y-меткой) рядом с этим устройством в списке привязанных. Внутреннее наименование маркера в коде — `pending-push`, но user не видит этот term.
- **FR-047**: При входе в Settings конкретного устройства, если для него есть pending, MUST показываться **баннер** с user-visible текстом «Есть изменения, которые ещё не отправлены» с действиями «Отправить сейчас» / «Отменить изменения».
...
- **FR-057 (Discard confirmation)**: Действие «Отменить изменения» (FR-047) MUST требовать confirmation dialog с user-visible текстом вида «Удалить изменения, которые ещё не отправлены? Изменения нельзя будет восстановить.». Обоснование (ux-quality checklist 2026-05-14): pending changes могут представлять часы работы пользователя; в отличие от push (который reversible через новую запись), discard уничтожает state без возможности восстановления. Push же НЕ требует confirmation (FR-040, FR-015 — push reversible). Cancel merge UI НЕ требует confirmation (FR-055 — pending preserved).
...
- **SC-001b (apply confirmation — second indicator)**: После того как Managed обновил `/state.appliedConfigUpdatedAt` (т.е. реально применил), editor-UI MUST показать **второй значок** с user-visible надписью «Применено на телефоне ✓» рядом с тем же устройством. Это отдельный от SC-001 индикатор — пользователь видит две фазы: «Отправлено ✓» (SC-001) → «Применено на телефоне ✓» (SC-001b). Жёсткого бюджета времени между ними нет; если целевое устройство долго offline — значок «Применено» появится позже (когда устройство догонит).
...
- **SC-004a (cold start with last-applied)**: После process death first frame UI Managed-телефона MUST отрисовываться с **last-applied-config из Room** (без сетевых обращений до first frame) и укладываться в бюджет SC-007 спека 007: **≤ 650 ms (p95)**. Пользователь Managed-телефона видит свой обычный экран мгновенно при старте лаунчера.
