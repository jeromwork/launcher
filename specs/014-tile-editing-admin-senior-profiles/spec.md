...
Admin сопряжён с Managed устройством бабушки (через спеку 007). Admin в своём Workspace проваливается в target editing — открывает Editor для бабушкиного Simple Launcher'а. **Видит структурный рендер** её layout'а (примерно 2×3 сетка, такая же как у неё) с **универсальным mainstream edit UX** (long-press, drag-and-drop, snackbar undo) + **visual indicators remote editing** (рамка 4dp + banner «Редактируешь телефон Маши»). Picker фильтруется по target preset — Simple Launcher target имеет 3 вкладки (App / Contact / Document; Widget/Action скрыты per FR-019). Добавляет/удаляет/перемещает плитки → push'ит через ConfigEditor → изменения уходят в Firestore. Дальнейшая синхронизация на бабушкин телефон (FCM + applier) — out of scope F-014, governed спекой 007/008.
...
**Independent Test**: эмулятор admin (Workspace) + эмулятор Managed (Simple Launcher) сопряжены. Admin Editor → выбрать paired device → видна структура layout'а target'а + **рамка вокруг grid** + banner «Редактируешь телефон Маши» с кнопкой «← Назад». Operations работают по **универсальным mainstream правилам** (long-press, drag-and-drop, snackbar undo) — Q4 cancellation 2026-05-29: edit mode UX тот же что в admin Workspace. Picker фильтруется по target preset — Simple Launcher target скрывает Widget/Action вкладки (FR-019). Изменения push'атся в Firestore без conflict (или conflict resolution через optimistic concurrency); end-to-end timing до Managed устройства — out of scope F-014, governed спекой 007/008.

**Acceptance Scenarios**:

1. **Given** admin сопряжён с Managed бабушки, **When** admin открывает Editor для этого target'а, **Then** рендерится структурный layout (2×3 grid) с **универсальным edit mode UX** (mainstream Android-конвенции: long-press, drag-and-drop, jiggle) + **рамка вокруг grid + top banner «Редактируешь телефон Маши» + кнопка «← Назад»** (visual indicators remote editing).
2. **Given** admin в target Editor, **When** тапает «+» в пустой ячейке, **Then** открывается unified picker с **3 вкладками** (Application / Contact / Document) — Widget и Action **скрыты**, потому что target preset = Simple Launcher (per FR-019).
3. **Given** admin в target Editor, **When** drag'ает плитку в zone «×» сверху, **Then** плитка удаляется, появляется snackbar «Удалено: <label>. Отменить» (8 sec) — mainstream Material 3 pattern.
4. **Given** admin в target Editor, **When** тапает «Назад» в banner'е (или «Готово» / tap-anywhere вне выделения), **Then** возвращается в свой Workspace, изменения сохранены в target ConfigDocument'е, push отправлен в Firestore.
5. **Given** admin изменил target ConfigDocument, **When** push в Firestore успешен (без conflict) — изменения **в главной ветке** конфигурации. Дальнейшая синхронизация на бабушкино устройство (FCM delivery + applier на её стороне) — out of scope F-014, governed спекой 007 (push channel) + 008 (config applier). F-014 не measures end-to-end timing.
...
2. **Given** бабушка прошла challenge, **When** challenge success, **Then** попадает в edit mode своего Simple Launcher'а — **универсальный mainstream edit UX** (long-press, drag, snackbar undo, jiggle) **без banner «Редактируешь чужое»** (это её собственный target). Per FR-012: edit mode universal across all contexts.
3. **Given** бабушка в edit mode, **When** drag'ает плитку на другую ячейку, **Then** плитка перемещается (snap-to-cell), Material 3 анимация.
4. **Given** бабушка в edit mode, **When** drag'ает плитку в zone «×» сверху, **Then** плитка удаляется, snackbar «Удалено. Отменить» (8 sec).
5. **Given** бабушка завершила edit, **When** тапает «Готово» (или back / tap-anywhere), **Then** edit mode заканчивается, **возвращается в use-mode rendering** Simple Launcher'а (senior-safe: ≥56dp tap targets, no jiggle, no hidden gestures). ConfigDocument сохранён локально (зашифровано после F-5).
...
- **SC-003 (Cross-device sync — functional outcome only)**: Admin'ский edit **успешно push'ится в Firestore** без ошибок (HTTP success response), либо optimistic concurrency conflict разрешается через FR-016/FR-017 (UI dialog «Обновить / Перезаписать»). **Никакое timing не measured** — cross-device latency на бабушкином home зависит от network conditions, FCM delivery, OEM battery management, и **выходит за пределы responsibility F-014**. Подтверждается integration test на successful Firestore write + conflict resolution branch.
