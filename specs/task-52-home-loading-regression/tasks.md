...
- [x] **T063** Вызвать skill `pre-pr-backlog-sync` ДО `gh pr create`: обновит backlog AC под текущее состояние (auto-checklist count'ы, auto-deferred list из этого tasks.md), переведёт TASK-52 → `Verification` (если deferred AC остались `[ ]`) или `Done` (если все физические gates успели пройти). Trace: CLAUDE.md HARD RULE pre-PR sync.
