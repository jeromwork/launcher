...
- **FR-014**: `RecoveryPassphraseSetupScreen` Composable MUST:
  - Два input field (passphrase + confirmation) с `KeyboardOptions(keyboardType = Password)`.
  - Autofill hints: `Modifier.semantics { contentType = ContentType.NewPassword }`.
  - Confirm button disabled пока passphrase < 8 символов OR не совпадают.
  - Senior-safe: text size ≥ 18sp, tap targets ≥ 56dp, contrast ≥ 4.5:1 (checklist-elderly-friendly).
  - **Neutral copy**: «придумайте пароль для восстановления» (НЕ «придумайте пароль для Google»).
  - **Blocking upload with progress UI**: после tap «продолжить» screen остаётся открытым со spinner + text «сохраняем ваш ключ для восстановления...» пока `RecoveryKeyBackup.uploadBlob()` не вернёт `Outcome.Success` ИЛИ 3-я неудача подряд. На 3-й неудаче — explicit dialog «облачный backup не удался (нет сети / квота). Продолжить без облачной копии? Если потеряете телефон — данные пропадут.» с buttons «отмена / продолжить локально». Если пользователь выбрал «продолжить локально» — flag `recoveryBackupDeferred=true` в DataStore, в Settings UI появляется «попробовать backup ещё раз». Reasoning: блокирующий upload предотвращает silent data loss (Q-C).
  - **Slow-device progress**: на Argon2id derivation (≤3s P95, может быть до 10s на старом hardware) — отдельный spinner + text «проверяем ваш пароль...» (Q-L).

- **FR-015**: `RecoveryPassphraseEntryScreen` Composable MUST:
  - Один input field с `ContentType.Password` autofill hint.
  - Inline error «неверный пароль» с счётчиком оставшихся попыток (e.g., «осталось 3 попытки»).
  - После 5 неверных подряд — automatic navigation на `RecoveryFallbackScreen`.
  - Neutral copy.
  - **Rate-limit per-identity** (НЕ глобальный): счётчик хранится в DataStore с key `recovery-attempts/{stableId}`, переживает process kill. Wipe в Fallback обнуляет только свой `stableId` namespace. Reasoning: глобальный счётчик создаёт DoS — attacker через identity A блокирует identity B на том же устройстве (Q-B).
  - **Slow-device progress** spinner на Argon2id derivation (как в FR-014).

- **FR-016**: `RecoveryFallbackScreen` Composable MUST:
  - Объяснение «начать заново = потеря старых зашифрованных данных».
  - Двойное подтверждение (button «начать заново» → dialog «вы уверены? старые конфиг и контакты пропадут» с «отмена» / «да»).
  - Senior-safe destructive button styling (border, color tonal).
...
- **SC-007**: `core/keys/src/commonMain/` под grep'ом не содержит слов `Google|Firebase|OAuth|Apple|Phone|Email|Sub|IdToken`. Fitness function автоматизирован в Konsist / Detekt правиле.
