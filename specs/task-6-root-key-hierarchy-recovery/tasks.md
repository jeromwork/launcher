...
- [x] **T681** ✅ **UPLOAD half SUCCESS 2026-06-30 on Xiaomi 11T** (cross-device recovery still pending — see below). E2E flow now works end-to-end through the Cloudflare deploy: install → wizard (Senior → Sign-In via Credential Manager with `g.jeromwork@gmail.com`) → identity worker `POST /init-claim → Ok` → token cache invalidated → `RecoveryPassphraseSetupScreen` rendered → passphrase entered → Готово → Argon2id derive (198ms) → AEAD wrap → `POST /backup → 200 stored`. Verified KV blob exists at `backup/GUOJK0YapWNTgOi2twrDr0I3ov43/v1.json` (routing key is currently Firebase sub via soft-fallback — T681-FOLLOWUP wire change, see commit log) with all 7 wire-format fields valid (schemaVersion=1, stableId=UUID, salt 32B, kdfParams Argon2id-64MiB/3/1, ciphertext 48B, nonce 24B, createdAt ISO-8601). Strict `claims.stableId` enforcement gated behind Firebase custom-claim propagation latency — accepted as MVP soft-fallback per the commit body. **Cross-device second half** (factory-reset device A → install on device B → enter same passphrase → byte-equal recovery) still pending — requires either a second physical device or a factory-reset cycle on the same one. (US-2)
