...
- [ ] **T681** [deferred-physical-device] **SC-001 cross-device manual smoke** — **partial 2026-06-30 on Xiaomi 11T**: install → walk wizard (Senior → Sign-In via Google) → **F-5 RecoveryPassphraseSetupScreen rendered ✓** → user types passphrase → tap Готово → `RecoveryFlow.performSetup` runs → `WorkerRecoveryKeyBackup.uploadBlob` POSTs to live `launcher-backup-v1.gpt1-jeromwork.workers.dev/backup`. Worker returns **403 STABLE_ID_MISMATCH** because the Firebase ID-token at this point still carries `claims.uid` only (the `claims.stableId` custom claim minted by identity worker requires a token-refresh round; the new token after `setCustomAttributes` was not yet pulled by the client when upload fired). **NOT a task-6 spec defect** — domain layer + Worker + adapter are correct; this is a token-refresh ordering bug at the F-4 ↔ F-5 boundary. Tracked as **T681-FOLLOWUP**: after `InitClaimClient.initClaim()` Success, app must force `FirebaseAuth.currentUser.getIdToken(true)` to fetch the refreshed token containing the new custom claim before any backup call. Cross-device part (factory reset → recover → byte-equal) blocked until T681-FOLLOWUP closes.
