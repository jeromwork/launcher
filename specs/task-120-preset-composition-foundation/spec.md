...
- **FR-027** (Capability model, owner directive session 2.5 clarify Q6 reframe): вводится **три domain port'а** для outstanding cloud/runtime state gating без coupling через JSON:
  ...
  - **Fitness function #9** (new): PresetValidator тесты покрывают три сценария — правильный ordering (SignIn provides CloudSession → HealthForward requires CloudSession), malformed (HealthForward перед SignIn → `ValidationError.CapabilityMissing`), optional path (Component с `requires = emptySet()` не блокируется независимо от ordering).
...
- **FR-030** (Identity-boundary, owner clarify 2026-07-10 post-analyze): Pool declarations MUST be identity-free per rule 9. No `pairingId`, `phoneNumber`, `userUid`, `accountEmail` fields in any `Component` subtype schema. Identity-bound values resolved at Provider apply-time via dedicated ports (`PairingService.currentAdmin()`, future `ContactsResolver`, `SubscriptionEntitlement`). If port returns null (e.g. pairing not yet established) → `Outcome.Failed(FailReason.PairingNotEstablished)` (new FailReason category); Wizard offers nested pairing step (deferred to TASK-67 pairing spec). Preset stays shareable per rule 9; Profile carries identity locally per device-only discipline. **Anti-pattern to refuse**: placeholder strings in Pool (`"PAIR-PLACEHOLDER"`, `"phoneNumberSlot"`) — replaced by port-based resolution.
...
- **PairingService**: port для resolve identity-bound pairing target. `suspend fun currentAdmin(): PairingId?`. Реализация в TASK-67 pairing spec; MVP foundation имеет только interface + FakePairingService для тестов. Sos Provider дёргает эту port'у на apply-time — если pairing null, возвращает `Outcome.Failed(PairingNotEstablished)`.
