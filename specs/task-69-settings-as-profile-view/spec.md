...
Задача продолжает линию TASK-120 (модель Component/Preset/Profile) → TASK-126 (визард на композиции) → TASK-127 (ECS + домашний экран). На момент написания карточки (30.06.2026) этой линии ещё не существовало, поэтому словарь карточки устарел; спека опирается на фактическое состояние кода:

- **Пресет уже несёт поле `settingsMap`** ([Preset.kt:41-47](../../core/src/commonMain/kotlin/com/launcher/preset/model/Preset.kt#L41-L47)): `poolRef`, `categoryKey`, `settingsIcon`, `sensitivity`, `paramsOverride`. Все три bundled-пресета его заполняют.
...
- **FR-002**: Для каждой записи `settingsMap` система MUST найти соответствующий компонент профиля по `poolRef`; при отсутствии — строку пропустить, экран отрисовать без ошибки.
- **FR-003**: Строки MUST группироваться по `categoryKey` записи `settingsMap`.
- **FR-004**: Каждая строка MUST показывать текущее значение компонента и его состояние (применено / ожидает / ошибка / состояние неизвестно).
...
- **FR-013**: Компоненты со статусом `Unverifiable` MUST отображаться как «состояние неизвестно» с возможностью применить повторно, а не как настроенные.
...
- **Profile** — состояние устройства: список компонентов с их значениями и статусами. Источник правды для экрана.
...
- **Component** — единица настройки (размер шрифта, тема, плитка, SOS, панель, роль лаунчера, системная панель).
- **ComponentStatus** — состояние применения: ожидает / применено / ошибка / пропущено / непроверяемо. Впервые проецируется в интерфейс построчно.
