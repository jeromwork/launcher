...
- **FR-019b**: `DocumentViewer` MUST предоставить **TalkBack-friendly альтернативу** pinch-to-zoom — постоянно видимые кнопки `+` и `−` (tap-target ≥ 56dp каждая, контраст ≥ 4.5:1) и double-tap zoom. Кнопки visible **независимо** от состояния TalkBack (не скрываются для full-vision users — gesture'ы и кнопки coexist). Без этого fallback'а DocumentViewer не accessible для бабушек со включённым screen reader'ом.
...
### Functional Requirements — Local storage protection

- **FR-025**: Decrypted media files в `LocalMediaStore` (`Context.filesDir/private-media/<uuid>`) MUST быть исключены из cloud-backup (Google Drive Auto Backup) и device-transfer (Samsung Smart Switch, etc.) через `res/xml/data_extraction_rules.xml` с правилами `<cloud-backup>` и `<device-transfer>` exclude. Без этого расшифрованные фото паспортов и медкарт бабушки автоматически уйдут в её Google account — это **critical PII leak** в иначе тщательно защищённой системе.
