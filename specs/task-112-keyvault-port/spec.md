...
- **FR-006**: `PassphraseRecovery` adapter MUST соответствовать TASK-6 semantics: `deriveRoot() = Argon2id(passphrase, salt, params)` с параметрами `Argon2Params.V1(memory=64MiB, iterations=3, parallelism=1)`. Regression с TASK-6 — обязательный test.
...
- **`KeyVault`** (port, `:core:keys/api/`) — крипто-контракт для всего домена. Operation-on-vault паттерн: ключи не покидают vault.
...
- **`RecoveryStrategy`** (port) — pluggable адаптеры для восстановления `root_key`.
- **`PassphraseRecovery`** (adapter) — MVP реализация соответствующая TASK-6.
- **`Argon2Params`** (frozen data class) — `V1(memory=64MiB, iterations=3, parallelism=1)`. Versioned.
- **`VaultException`** (sealed hierarchy) — все ошибки KeyVault.
- **`AndroidKeyVault`** (adapter, `:core:keys/impl/androidMain/`) — Android platform-specific. Keystore для `root_key`, libsodium-kmp для всего остального.
- **`FakeKeyVault`** (adapter, `:core:keys/impl/commonTest/`) — in-memory для тестов downstream features.
- **`RootKey`** (internal class, `family.keys.impl.*`) — 32 байта root material. НЕ пересекает port boundary.
...
- **SC-002 [backlog]**: Existing user data (encrypted config from TASK-6 timeframe) читается после апдейта без миграции — backward compat. Verified через integration test с pre-migration fixture.
- **SC-003 [backlog]**: Recovery flow через passphrase работает identically to TASK-6 (Setup → Entry → Fallback screens). Regression test прогоняется на эмуляторе.
...
- TASK-6 (Root Key Hierarchy + Owner Recovery) остаётся as-is — passphrase-based recovery для MVP. TASK-112 обёртывает существующее в `PassphraseRecovery` adapter, не переписывает.
