...
- [x] **T040** [US1] Update `core/src/androidMain/assets/wizard/wizard-manifests/simple-launcher.json` per contracts/simple-launcher-manifest.md: `body.autoOrder: false`; `body.steps: [...]` with 3 explicit entries — ROLE_HOME (Required, canSkip:false override), tileSet (Required), POST_NOTIFICATIONS (Required, canSkip:true override). (FR-001, FR-002, FR-003, FR-004, SC-001, Plan Phase 4) — *Originally 4 entries including `Custom("pair-admin") Optional`. Reverted to 3 on 2026-06-25 per constitution amendment 1.10 (no Custom step). pair-admin returns as `SystemSetting` at TASK-8.*
...
- [x] **T045** [multi] Manifest content validation test in `core/src/androidUnitTest/kotlin/com/launcher/adapters/wizard/SimpleLauncherManifestTest.kt`: load `simple-launcher.json` via real `BundledConfigSource`; assert 3 steps present in expected order with expected per-profile canSkip overrides; assert every step's `refId` resolves to an existing pool entry (no dangling refs); assert all referenced localization keys exist in strings resources (CI fitness function). Requires: T040, T041, T043. (FR-001, FR-002, Plan Phase 4) — *Originally asserted 4 steps. Updated to 3 on 2026-06-25 with Phase-5 revert.*
...
## Phase 5: PairAdmin Custom step + DI wiring — **REVERTED 2026-06-25**

**Status**: ❌ **REVERTED per constitution amendment 1.10.** All Phase-5 work removed from the codebase 2026-06-25 after device verification surfaced the underlying architectural problem.

**What was reverted and why.** Phase-5 introduced `StepType.Custom(name)` + `CustomStep` + `CustomStepHandler` + `PairAdminCustomStepHandler` to bolt the spec-007 pairing flow onto the wizard. The implementation fire-and-forget-launched `PairingActivity` on engine step entry — no user UI, no skip path, no `CheckSpec`-based state read — violating Constitution Article VII §11 (wizard = projection of config, not source of truth) and §15 (multi-platform adapter seam through declarative `CheckSpec` / `ApplySpec`). The `Custom` step type itself was an F-3 placeholder added without use case (CLAUDE.md rule 4 violation — abstraction for hypothetical future). Constitution amendment 1.10 retires the placeholder and forbids the pattern; pair-admin will return as a standard `SystemSetting` step at TASK-8 time, with `CheckSpec.PairAdminLink` reading `activeAdminLinkCount` from the registry and `ApplySpec.PairAdminIntent` dispatching to the pairing UI via the generic intent handler. See [`docs/dev/project-backlog.md`](../../docs/dev/project-backlog.md) TODO-TASK7-005.

- [N/A] **T046** ~~Add `CustomStep` class + `CustomStepHandler` port~~ — file deleted 2026-06-25.
- [N/A] **T047** ~~`PairAdminCustomStepHandler`~~ — file deleted 2026-06-25.
- [N/A] **T048** ~~Koin DI wiring for Custom step + customStepHandlers map~~ — wiring removed 2026-06-25.
- [N/A] **T049** ~~`PairAdminStepIntegrationTest`~~ — file deleted 2026-06-25.

**Checkpoint Phase 5**: REVERTED. pair-admin step removed from `simple-launcher.json` manifest (manifest now 3-step: ROLE_HOME + tileSet + POST_NOTIFICATIONS). Returns at TASK-8 as standard `SystemSetting` step.
...
- [ ] **T066** [US4] [deferred-physical-device] Real pairing 2-device flow: phone A in primary user mode + phone B with admin app stub (mock for TASK-8) → QR scan from A targeting B's QR → `LinkRegistry.activate()` succeeds → trust handshake recorded. (SC-005, Plan Phase 7) — ⚠️ **BLOCKED 2026-06-25**: (a) pair-admin step removed from manifest 2026-06-25 with `StepType.Custom` retirement (constitution amendment 1.10); returns at TASK-8 as `SystemSetting`. (b) PairingActivity itself still crashes на arm64 с `UnsatisfiedLinkError crypto_core_ristretto255_add` — передано в **TASK-51** (libsodium native lib bug, spec 020 / F-CRYPTO).
...
### Регрессии в TASK-7 own — РАЗОБРАНЫ 2026-06-25
- **TASK-54** — senior-warm Light theme contrast fail (T060). Текст WizardActivity нечитаем на белом фоне. Закрыта как "won't fix в MVP" per owner decision 2026-06-25 (Article II §8: MVP polish через JSON не через код).
- ~~**TASK-55**~~ — Phase-5 PairAdmin Custom step auto-launches PairingActivity. Был выделен в TASK-55 как "needs UI confirm follow-up", но более глубокий анализ показал что это симптом более крупной архитектурной проблемы (`StepType.Custom` сам по себе). Решено через **Constitution amendment 1.10**: `Custom` step type удалён, pair-admin step убран из manifest, возвращается через `SystemSetting` + `CheckSpec.PairAdminLink` / `ApplySpec.PairAdminIntent` при TASK-8 (см. TODO-TASK7-005 в project-backlog.md). TASK-55 закрыть как obsolete.
