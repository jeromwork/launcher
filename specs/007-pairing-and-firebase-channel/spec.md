...
|---|-----------|------------|
| C1 | **FCM-механизм доставки push'ей** (FR-016): Cloud Function на Firestore-trigger / Firebase Extension / клиент-side topic-broadcast от admin'а? | **Resolution (revised 2026-05-11 b):** **client-side FCM с topic-per-link** (Variant C). Admin после успешного write в `/links/{linkId}/config` или `/links/{linkId}/commands/{cmdId}` **сам** инициирует доставку push'а тем способом, который не требует серверной инфраструктуры: подписка OLD на FCM topic `link-{linkId}` (через `FirebaseMessaging.subscribeToTopic()`), admin отправляет push через **FCM HTTP v1 send-API** с использованием **short-lived OAuth access token**, полученного через `GoogleSignIn`/`AppCheck` flow на стороне admin-приложения (нет server-key в APK). Подробности механизма выбора токена — в research.md. **Why:** (а) **Spark-plan совместимо, без Blaze, без привязки карты** — соответствует текущему статусу проекта; (б) Cloud Function (Variant A) требует Blaze plan, что нежелательно для MVP; (в) wire-format push'а (`contracts/fcm-payload.md`) идентичен — переключиться на Variant A позже = заменить отправителя, не клиента. **Cost:** admin-приложение получает OAuth scope на отправку FCM (security review в plan.md); подписка/отписка от topic при pairing/revoke. **Exit ramp:** при росте трафика или появлении server-side операций (cron-чистка expired `/pairings`, антифрод) — апгрейд на Blaze + переключение C1 на Variant A. Wire-format не меняется. **Why was C1 revised:** изначально (2026-05-11 a) был выбран Variant A — это требовало Blaze plan; project owner подтвердил отсутствие карты, поэтому Variant C принят как стартовая позиция. |
...
- **Спек 008** наследует выбор C1 (client-side FCM, topic-per-link) — все push'и для config-sync пойдут тем же путём, на Spark plan. Если в 008 появится необходимость в серверных операциях (cron, fan-out, server-side validation) — апгрейд на Blaze и переключение C1 на Variant A.
...
- **Roadmap** — спек 007 совместим с **Spark plan** (no card binding required) после revision C1 на Variant C; Blaze апгрейд отложен до появления серверных потребностей.
...
- **FR-016**: Лаунчер MUST обрабатывать silent data-push сообщения от FCM как сигнал «перечитай config/commands». Push'и отправляет **admin-приложение клиент-сайд** через FCM HTTP v1 send-API на topic `link-{linkId}` после успешного write в `/links/{linkId}/config` или `/links/{linkId}/commands/{cmdId}` — см. C1 (revised 2026-05-11 b). OLD подписан на topic `link-{linkId}` после pairing. Контракт payload — `contracts/fcm-payload.md`.
...
- Cloud Function / Firebase Extension для отправки FCM **не нужны** (см. revised C1) — выбран Variant C: admin-приложение само вызывает FCM HTTP v1 send-API на topic `link-{linkId}` после успешного write. Альтернативы (Variant A — Cloud Function, Variant B — Trigger Email/FCM Extension) остаются как exit ramp при появлении server-side требований.
