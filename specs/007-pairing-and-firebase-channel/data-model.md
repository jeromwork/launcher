...
- Не путать с `managedDeviceId` (UUIDv4 из DataStore).
- Firebase Auth UID — короткоживущий; `managedDeviceId` — стабильный.
...
- Same shape as `ManagedIdentity`; разный role discriminator на DI-уровне (admin-mode/Managed-mode Koin scopes).
...
data class Link(
  ...
  val managedDeviceId: String,               // UUIDv4 from DataStore
  val managedDeviceFirebaseUid: String,      // current Firebase Auth UID; меняется при reinstall (см. research.md)
...
enum class PairingType {
  AdminManagedLink,           // spec 007 — единственный текущий subtype
...
В wire-format `/pairings/{token}` `pairingType: String?` (default `"admin-managed-link"` если отсутствует — backward-compat для существующих токенов).
...
sealed interface DocPath {
  ...
  data class Devices(val managedDeviceId: String) : DocPath {
    override val rawPath = "devices/$managedDeviceId"
  }
...
class PairingService(
  ...
  private val deviceId: DeviceIdProvider,                 // wraps DataStore (managedDeviceId)
  ...
  // Managed-side
  suspend fun startPairingAsManaged(): Result<PairingToken, PairingError>
  suspend fun cancelPairingAsOld(): Result<Unit, PairingError>
  suspend fun confirmConsentAsManaged(): Result<Link, PairingError>     // → linkRegistry.activate + FCM subscribe
  suspend fun declineConsentAsManaged(): Result<Unit, PairingError>
  ...
  // В 007 единственный result subtype — Link (admin↔Managed).
...
```text
ManagedIdentity, AdminIdentity (value classes, opaque)
       │
       ├──► Link.adminId / Link.managedDeviceFirebaseUid (ids only — no nested in wire)
       │
...
- При reinstall — новый UID; `managedDeviceId` в DataStore выживает только если backup/restore включён (Android Auto Backup); по умолчанию reset → новый pairing.

**TODO в `IdentityCache.kt`**: «при добавлении named auth (OWD-2) — linkWithCredential сохранит UID; managedDeviceId как stable id остаётся».
...
1. **Идентичности** (`ManagedIdentity`, `AdminIdentity`) — обёртки над Firebase Auth UID. Domain-типы, без зависимости от SDK.

2. **Pairing-токен** (`PairingToken`) — 6 символов, alphabet без `0/O/I/1`, single-use, TTL 5 мин.

3. **Link** — «связь admin ↔ Managed». Поля: `linkId` (random), `adminId`, `managedDeviceId`, `createdAt`. Хранится в `/links/{linkId}` Firestore.
...
- `managedDeviceId` — наш стабильный UUID, не теряется между запусками.
