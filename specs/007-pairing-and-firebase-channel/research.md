...
Cloudflare Worker — отдельный сервис (~50 LOC TS), hosted на бесплатном tier `*.workers.dev`. Worker хранит Firebase service-account JSON в Cloudflare Secrets, принимает HTTPS POST от admin/Managed клиентов, валидирует Firebase ID-token, авторизует через Firestore (`uid == links/{linkId}.adminId`), отправляет FCM data-message.
...
### Encoding (на стороне Managed — показ QR)
...
ZXing **остаётся** для **encode-стороны (генерации QR)** — pure Kotlin/Java, KMP-compatible, легче и без необходимости ML Kit на Managed-устройстве.
...
Anonymous Firebase Auth UID — **транзитный**: при reinstall app — новый UID. Но FR-001 требует `managedDeviceId` сохранялся между перезапусками.

### Решение

- `managedDeviceId` = UUIDv4, generated on first launch, stored in DataStore (FR-001).
- `IdentityProvider.signInAnonymous()` returns Firebase Auth UID.
- В `/devices/{managedDeviceId}` Firestore-doc записываем **наш** `managedDeviceId` как ключ + поле `firebaseAuthUid`. Pairing идентифицирует устройство по нашему `managedDeviceId`.
- В `/links/{linkId}.managedDeviceFirebaseUid` пишем актуальный Firebase UID. Security Rules позволяют Managed писать в state по `request.auth.uid == managedDeviceFirebaseUid`.
- При reinstall: новый `managedDeviceId` → пользователь должен **новый pairing**. Документировано как known limitation (OWD-2 exit ramp описывает миграцию к named auth).

**TODO в `IdentityCache.kt`**: «при добавлении named auth (OWD-2 exit ramp) — `linkWithCredential` сохраняет UID; managedDeviceId остаётся unchanged как stable id».
...
Each `deleteSubcollection` — batched delete (100 docs / batch). Security Rules разрешают delete только owner-Managed.
...
Добавлено 2026-05-11 c (post-checklist), после замечания project owner'а: pairing-flow универсален и будет использоваться много где, не только admin↔Managed.

### Будущие use case'ы trust-pairing

| Use case | Spec | Result-тип pairing'а |
|---|---|---|
| admin↔Managed | 007 (текущий) | `LinkBootstrap` |
...
1. **wire-format `/pairings/{token}`**: добавить optional поле `pairingType: String?` (default `"admin-managed-link"` если отсутствует — backward-compat). Реализуется в `contracts/pairing-token.md` как additive change (schemaVersion остаётся 1).
...
- **Применение `/config`** к раскладке Managed'а — спек 008.
