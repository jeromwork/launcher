...
- [X] T510 Create `Action`, `ActionPayload` (sealed with 9 variants per data-model §ActionPayload), `WhatsAppCallKind`, `YouTubeTarget`, `SettingsTarget` in `core/src/commonMain/kotlin/com/launcher/api/action/Action.kt`. Include `SUPPORTED_SCHEMA_VERSION = 1`, `MAX_FALLBACK_DEPTH = 2`. *(FR-Action-1; data-model §Action; plan §Architecture)*
- [X] T511 Create `ProviderId` value class with 8 constants and `fromWire(String)` validator (regex `[a-z][a-z0-9_-]{1,31}`) in `core/src/commonMain/kotlin/com/launcher/api/action/Action.kt`. *(Clarification C1; data-model §ProviderId; research R1)*
- [X] T512 Create `DispatchResult` (sealed: `Ok`, `BlockedByPolicy`, `ProviderUnavailable`, `Failure`), `BlockReason` enum, `UnavailabilityHint` enum in `core/src/commonMain/kotlin/com/launcher/api/action/Action.kt`. *(Clarification C1; data-model §DispatchResult)*
- [X] T513 [P] Create `ActionDispatcher` interface (port) in `core/src/commonMain/kotlin/com/launcher/api/action/ActionDispatcher.kt`. Single method: `suspend fun dispatch(action: Action): DispatchResult`. *(plan §Architecture port-adapter shape)*
- [X] T514 [P] Create `ProviderRegistry` interface, `ProviderState`, `ProviderAvailability`, `InstallHint`, `NotApplicableReason` in `core/src/commonMain/kotlin/com/launcher/api/action/ProviderRegistry.kt`. *(data-model §ProviderRegistry types)*
- [X] T515 Create `ActionWireFormat.kt` in `core/src/commonMain/kotlin/com/launcher/api/action/` — kotlinx-serialization wiring (custom serializer for `ProviderId`, polymorphism for `ActionPayload` via `kind` discriminator). Expose `fun encode(action: Action): String` and `fun decode(json: String): Action`. *(contracts/action-wire-format.md; plan §Test strategy)*
- [X] T516 [US-FORWARD-COMPAT] Add `migrateLegacyAction(legacyJson: String): Action?` in `ActionWireFormat.kt`. Handles old spec 003 mock format (`type: "whatsapp_call"` etc.) → new `Action` shape. Marker `LEGACY-BRIDGE-EXPIRES-IN-SPEC-006` for grep / Konsist. *(spec §6.3; Clarification C5)*
- [X] T517 Add `ProjectEvent.ActionDispatched` data class extending existing `ProjectEvent` sealed class in `core/src/commonMain/kotlin/com/launcher/api/ProjectEvent.kt`. Fields: `providerId`, `resultKind: ResultKind`, `fallbackUsed`, `timestampMs`. *(contracts/diagnostics-events-v2.md)*

### Tests for Phase 2 (run before any handler work)

- [X] T520 [P] [FOUNDATION] Roundtrip test: `ActionWireFormatTest` covers every `ActionPayload` variant (incl. `Custom` empty/full, null fallback, 2-deep fallback). 65 tests in `core/src/commonTest/kotlin/com/launcher/api/action/ActionWireFormatTest.kt`. Found and fixed two contract bugs: `WhatsAppCall.kind` collision with `classDiscriminator` (renamed wire-name to `callKind`), and legacy-bridge new-format detection (now uses `providerId` presence, not `schemaVersion`). *(spec §8.2 fitness function 2)*
- [X] T521 [P] [FOUNDATION] Test fixtures for roundtrip: 14 JSON files (one per payload variant + 3 youtube targets + 2 custom variants + fallback-chain) in `core/src/commonTest/resources/fixtures/action-wire-format/`. `ActionWireFormatFixtureTest` (androidUnitTest) parses each file and asserts equality to expected `Action`. README listing enforced by `fixture_directoryListedInReadme` test. *(Clarification C4)*
- [X] T522 [P] [FOUNDATION] Forward-compat tests: `unknownProviderId_parses_OK_andDispatchSurfacesUnknown` + `unknownPayloadKind_failsAtParse` + `futureSchemaVersion_parses_butIsRejectedAtDispatchTime` + `unknownPayloadField_isIgnored_perIgnoreUnknownKeys` in same file. *(Clarification C1; contracts/action-wire-format.md §Forward compatibility)*
- [X] T523 [P] [FOUNDATION] Backward-compat fixtures for spec 003: 5 `legacy-spec003-*.json` files (whatsapp-call voice/video, whatsapp-message, open-app, placeholder) in fixtures dir. `ActionWireFormatFixtureTest` asserts each file → expected `Action` (or null for placeholder). *(spec §4.1.5)*
- [X] T524 [P] [FOUNDATION] `FakeActionDispatcher` in `core/src/commonTest/kotlin/com/launcher/test/FakeActionDispatcher.kt`: records calls, returns programmable `DispatchResult`. *(plan §Test strategy fake-adapter; CLAUDE.md §6)*
- [X] T525 [P] [FOUNDATION] `FakeProviderRegistry` in `core/src/commonTest/kotlin/com/launcher/test/FakeProviderRegistry.kt`: programmable per-provider availability + Flow updates. *(plan §Test strategy fake-adapter)*
...
- [X] T541 [P] [US-502] `PhoneHandler` in `core/src/androidMain/.../handlers/PhoneHandler.kt`. Handles `Phone(number)` → `Intent(ACTION_DIAL, Uri.parse("tel:$number"))`. **No** `ACTION_CALL`. *(US-502; spec §7.5)*
  - **Спек 010 FR-012 extends this (T058)**: when `CALL_PHONE` is granted (user explicitly consented via spec-010 `CallConfirmationDialog`), handler now fires `Intent.ACTION_CALL` directly to save the user one tap (SC-003 «2 taps to call»). Falls back to `ACTION_DIAL` when permission missing. `PhoneHandlerTest` (T571) inverted accordingly. See [spec 010 FR-012](../010-setup-assistant/spec.md).
