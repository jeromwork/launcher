...
- [ ] T521 [P] [FOUNDATION] Test fixtures for roundtrip: create one JSON file per variant in `core/src/commonTest/resources/fixtures/action-wire-format/` (e.g. `whatsapp-message-v1.json`, `phone-call-v1.json`, `custom-v1.json`, `fallback-chain-v1.json` ✓). Currently only `fallback-chain-v1.json` and one legacy file exist. *(Clarification C4)*
