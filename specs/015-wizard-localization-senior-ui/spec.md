...
| C-3 | iOS / TV — F-3 должен закладывать поддержку сейчас? | **Не сейчас.** iOS будет в будущем, но F-3 не должен прогибаться под него. Компромисс: бизнес-логика KMP (commonMain) — даёт переносимость бесплатно; UI Android-only — iOS UI пишем с нуля когда iOS launcher материализуется. См. C-7. |
| C-4 | `core/wizard/` — pure state machine или CMP с готовым UI host? | **Pure state machine.** Без Compose dependency. Android Compose host живёт в `app/`. Когда iOS launcher материализуется — пишет свой host на SwiftUI поверх того же `commonMain` state. |
...
| C-7 | Module types для трёх модулей? | `core/wizard/` = KMP `commonMain + androidMain` (без iosMain). `core/localization/` = KMP `commonMain + androidMain` (без iosMain). `core/ui-senior/` = **Android-only** Compose library (НЕ Compose Multiplatform). iOS source sets добавляются в отдельной спеке когда первый iOS consumer материализуется. |
| C-8 | String-table библиотека? | **moko-resources.** iOS-aware с дня 1 (готова к будущему iosMain без рефакторинга), mature, generates typed accessors, не требует Compose runtime. |
...
| C-15 | Lint rule «core/* → app/*» — implementation? | **Konsist** (Kotlin DSL, runs as JUnit test). Идиоматичен для Kotlin, низкий overhead. **Rationale смягчён**: rule = «structural cleanliness», не «load-bearing extraction prerequisite». |
| C-16 | Inter-module dependency directionality (`core/wizard/` → `core/ui-senior/` разрешено)? | **Directional граф.** `core/wizard/` зависит от `core/localization/` (через `StringResolver` port). `core/wizard/` **НЕ** зависит от `core/ui-senior/` (wizard остаётся pure logic, UI host собирает consumer). `core/ui-senior/` self-contained. |
...
- **FR-001**: System MUST содержать `core/wizard/` как **Kotlin Multiplatform (KMP) module** с source sets `commonMain` (бизнес-логика, доступна всем платформам) + `androidMain` (Android-specific adapters). `iosMain` source set **НЕ создаётся** в F-3 — добавится в отдельной спеке когда первый iOS consumer материализуется (per C-7).
...
- **FR-026**: System MUST содержать `core/localization/` как **KMP module** с source sets `commonMain + androidMain` (без `iosMain` сейчас — добавится когда первый iOS consumer материализуется, per C-7).
...
- **FR-033**: System MUST содержать `core/ui-senior/` как **Android-only Compose library** (`com.android.library` + Jetpack Compose, **НЕ** Compose Multiplatform). Per C-3 + C-7: iOS UI пишется с нуля в отдельной спеке когда первый iOS launcher материализуется (либо на SwiftUI, либо на CMP если он созреет). Inline TODO в `core/ui-senior/README.md`: «iOS UI primitives — `core/ui-senior-ios/` отдельный модуль ИЛИ CMP refactor когда первый iOS consumer материализуется».
...
- **FR-038**: Build MUST содержать automated check на базе **Konsist** (per C-15), который **fails** если любой файл в `core/wizard/`, `core/localization/`, `core/ui-senior/` импортирует класс из `com.eastclinic.app.*` (или whatever app package namespace). Per C-15: rationale — «structural cleanliness» (не «load-bearing extraction prerequisite»); rule полезен сам по себе как discipline tool независимо от extraction триггеров.
- **FR-038a (inter-core directionality)**: Per C-16, Konsist check MUST дополнительно verify directional граф между core модулями: `core/wizard/` MAY импортировать из `core/localization/` (через `StringResolver` port). `core/wizard/` MUST NOT импортировать из `core/ui-senior/` (wizard остаётся pure logic). `core/ui-senior/` MUST NOT импортировать ни `core/wizard/`, ни `core/localization/` (self-contained UI primitives).
...
- **A-11**: Per C-7: `core/wizard/` и `core/localization/` — KMP с **`commonMain + androidMain`** source sets (без `iosMain` сейчас). Это даёт переносимость бизнес-логики на iOS бесплатно (additive iosMain позже) + JVM unit-tests без эмулятора. Android-specific bits (DataStore, Android resource reader) — в `:app/androidMain` через port pattern.
- **A-12**: Per C-7: `core/ui-senior/` — **Android-only Compose library** (НЕ Compose Multiplatform). Premature CMP adoption отвергнут (rule 4 MVA): нет iOS consumer'а сейчас, CMP на iOS ещё имеет gaps в gesture handling, learning curve + альфа-глюки замедлят F-3 без выгоды. Когда iOS launcher материализуется — пишем UI с нуля (SwiftUI или CMP по обстоятельствам того момента).
- **A-13**: **Persistent storage** для checkpoint store, dismissed hints, UserPreferencesStore — DataStore (Android) в `:app/androidMain`. Не SharedPreferences (deprecated). Решение реализационное, не блокирует F-3 спеку.
- **A-14**: Per C-8: **moko-resources** — выбран для string tables. Mature, iOS-aware с дня 1, generates typed accessors, не требует Compose runtime для resolution.
- **A-15**: Per C-15: **Konsist** — выбран для lint rule. Идиоматичен для Kotlin, runs as JUnit test, низкий setup overhead.
