...
**Контекст**: Через 5-10 лет XChaCha20-Poly1305 или Argon2id могут стать устаревшими (например, появится требование post-quantum криптографии). F-5 должен быть готов к миграции, не блокировать её.

1. В облаке появляется новый клиент с новым алгоритмом (например, post-quantum AEAD).
