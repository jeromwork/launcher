...
- **FR-042**: При first-launch System MUST детектировать наличие Google Play Services через `GoogleApiAvailability.isGooglePlayServicesAvailable()`. Если результат ≠ `SUCCESS` И ошибка **non-recoverable** (`SERVICE_INVALID`, `SERVICE_MISSING` без `isUserResolvableError`), System MUST показать **hard-block screen** до wizard'a: «Это устройство не поддерживается. Для работы приложения нужны Google Play Services, которых нет на этом устройстве.» Кнопка «Понятно» закрывает приложение (`finishAffinity()`).
...
- **FR-044**: Если GMS-ошибка **user-resolvable** (`SERVICE_VERSION_UPDATE_REQUIRED`, `SERVICE_DISABLED`), вместо hard-block — `GoogleApiAvailability.getErrorDialog()` (системный диалог, который ведёт пользователя на установку/включение GMS). Это **soft-block** — после успешного resolve приложение продолжит wizard.
